...
namespace _cpo_impls {
  ...
  namespace _target_id {
    enum class _St {
      ...
      _edge_data_member,     // Tier 6: edge_data data member
    ...
    concept _has_native_edge_member =
          adj_list::is_edge_descriptor_v<std::remove_cvref_t<E>> && requires(G& g, const E& uv) {
            { (*uv.value()).target_id() };
          };
    ...
    template <typename G, typename E>
    concept _has_adl_descriptor = requires(const G& g, const E& uv) {
      { target_id(g, uv) };
    };
    ...
    concept _has_edge_list_descriptor =
          edge_list::is_edge_list_descriptor_v<std::remove_cvref_t<UV>> && requires(const UV& uv) {
            { uv.target_id() };
          };
    ...
    concept _is_tuple_like_edge =
          !adj_list::is_edge_descriptor_v<std::remove_cvref_t<UV>> &&
          !edge_list::is_edge_list_descriptor_v<std::remove_cvref_t<UV>> && !_has_edge_data_member<UV> &&
          requires { std::tuple_size<std::remove_cvref_t<UV>>::value; } && requires(const UV& uv) {
            { std::get<0>(uv) };
            { std::get<1>(uv) };
          };
    ...
    [[nodiscard]] consteval _Choice_t<_St> _Choose() noexcept {
      if constexpr (_has_native_edge_member<G, E>) {
        return {_St::_native_edge_member, noexcept((*std::declval<const E&>().value()).target_id())};
      } else if constexpr (_has_adl_descriptor<G, E>) {
        return {_St::_adl_descriptor, noexcept(target_id(std::declval<const G&>(), std::declval<const E&>()))};
        ...
        return {_St::_edge_list_descriptor, noexcept(std::declval<const E&>().target_id())};
      } else if constexpr (_has_edge_data_member<E>) {
        return {_St::_edge_data_member, noexcept(std::declval<const E&>().target_id)};
      } else if constexpr (_is_tuple_like_edge<E>) {
        return {_St::_tuple_like, noexcept(std::get<1>(std::declval<const E&>()))};
      } else {
    ...
    public:
      /**
             ...
             * 5. uv.target_id - edge_data data member (Tier 6)
             ...
             * - For tiers 5-7, uv can be edge_list descriptors, edge_data structs, or tuple-like types
             ...
             * - edge_data structs with direct data members
      ...
      requires(_Choice<std::remove_cvref_t<G>, std::remove_cvref_t<E>>._Strategy != _St::_none)
      {
        ...
        if constexpr (_Choice<_G, _E>._Strategy == _St::_native_edge_member) {
          ...
          return uv.target_id();
        } else if constexpr (_Choice<_G, _E>._Strategy == _St::_edge_data_member) {
          return uv.target_id;
  ...
  namespace _source_id {
    ...
    template <typename E>
    concept _has_adj_list_descriptor = adj_list::is_edge_descriptor_v<std::remove_cvref_t<E>> && requires(const E& uv) {
      { uv.source_id() };
    };
    ...
    [[nodiscard]] consteval _Choice_t<_St> _Choose() noexcept {
      if constexpr (_has_native_edge_member<G, E>) {
        return {_St::_native_edge_member, noexcept((*std::declval<const E&>().value()).source_id())};
      } else if constexpr (_has_member<G, E>) {
        return {_St::_member, noexcept(std::declval<G>().source_id(std::declval<const E&>()))};
      } else if constexpr (_has_adl<G, E>) {
        return {_St::_adl, noexcept(source_id(std::declval<G>(), std::declval<const E&>()))};
      } else if constexpr (_has_adj_list_descriptor<E>) {
        return {_St::_adj_list_descriptor, noexcept(std::declval<const E&>().source_id())};
      } else if constexpr (_has_edge_list_descriptor<E>) {
        return {_St::_edge_list_descriptor, noexcept(std::declval<const E&>().source_id())};
      } else if constexpr (_has_edge_data_member<E>) {
        return {_St::_edge_data_member, noexcept(std::declval<const E&>().source_id)};
      } else if constexpr (_is_tuple_like_edge<E>) {
        return {_St::_tuple_like, noexcept(std::get<0>(std::declval<const E&>()))};
      } else {
        return {_St::_none, false};
      }
    }
    ...
    public:
      /**
             ...
             * 6. uv.source_id - edge_data data member (Tier 6)
  ...
  namespace _edge_value {
    ...
    template <typename G, typename E>
    concept _has_value_fn = (!adj_list::is_edge_descriptor_v<std::remove_cvref_t<E>>) && requires(const E& uv) {
      { uv.value() };
    };
    ...
    concept _is_tuple_like_edge =
          !adj_list::is_edge_descriptor_v<std::remove_cvref_t<UV>> &&
          !edge_list::is_edge_list_descriptor_v<std::remove_cvref_t<UV>> && !_has_edge_data_member<UV> && requires {
    ...
    [[nodiscard]] consteval _Choice_t<_St> _Choose() noexcept {
      if constexpr (_has_member<G, E>) {
        return {_St::_member, noexcept(std::declval<G>().edge_value(std::declval<const E&>()))};
      } else if constexpr (_has_adl<G, E>) {
        return {_St::_adl, noexcept(edge_value(std::declval<G>(), std::declval<const E&>()))};
      } else if constexpr (_has_value_fn<G, E>) {
        return {_St::_value_fn, noexcept(std::declval<const E&>().value())};
      } else if constexpr (_has_adj_list_descriptor<G, E>) {
        // Default to false (safe) since we have conditional logic
        return {_St::_adj_list_descriptor, false};
      } else if constexpr (_has_edge_list_descriptor<E>) {
        return {_St::_edge_list_descriptor, noexcept(std::declval<const E&>().value())};
      } else if constexpr (_has_edge_data_member<E>) {
        return {_St::_edge_data_member, noexcept(std::declval<const E&>().value)};
      } else if constexpr (_is_tuple_like_edge<E>) {
        return {_St::_tuple_like, noexcept(std::get<2>(std::declval<const E&>()))};
      } else {
        return {_St::_none, false};
      }
    }
    ...
    public:
      /**
             ...
             * 6. uv.value - edge_data data member (Tier 6)
      ...
      requires(_Choice<std::remove_cvref_t<G>, std::remove_cvref_t<E>>._Strategy != _St::_none)
      {
        ...
        if constexpr (_Choice<_G, _E>._Strategy == _St::_member) {
          ...
          return std::forward<E>(uv).inner_value(std::forward<E>(uv).source().underlying_value(std::forward<G>(g)));
        } else if constexpr (_Choice<_G, _E>._Strategy == _St::_edge_list_descriptor) {
          return uv.value();
        } else if constexpr (_Choice<_G, _E>._Strategy == _St::_edge_data_member) {
          return uv.value;
