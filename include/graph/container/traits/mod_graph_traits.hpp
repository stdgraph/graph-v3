...
//  Edges:    std::deque (double-ended; random access with efficient front/back insertion)
...
//         The std::deque edge container provides random access iteration with
//         efficient insertion at both ends (unlike vector which is only efficient at back).
...
struct mod_graph_traits {
  ...
  using edge_type   = dynamic_out_edge<EV, VV, GV, VId, Bidirectional, mod_graph_traits>;
  using vertex_type = dynamic_vertex<EV, VV, GV, VId, Bidirectional, mod_graph_traits>;
  using graph_type  = dynamic_graph<EV, VV, GV, VId, Bidirectional, mod_graph_traits>;
