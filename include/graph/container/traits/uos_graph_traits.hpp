...
//  - Requires operator== and std::hash on VId, operator< on dynamic_edge (implemented in dynamic_graph.hpp)
//
...
struct uos_graph_traits {
  ...
  using edge_type   = dynamic_edge<EV, VV, GV, VId, Sourced, uos_graph_traits>;
  using vertex_type = dynamic_vertex<EV, VV, GV, VId, Sourced, uos_graph_traits>;
  using graph_type  = dynamic_graph<EV, VV, GV, VId, Sourced, uos_graph_traits>;
