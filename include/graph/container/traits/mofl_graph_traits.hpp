...
struct mofl_graph_traits {
  ...
  using edge_type   = dynamic_out_edge<EV, VV, GV, VId, Bidirectional, mofl_graph_traits>;
  using vertex_type = dynamic_vertex<EV, VV, GV, VId, Bidirectional, mofl_graph_traits>;
  using graph_type  = dynamic_graph<EV, VV, GV, VId, Bidirectional, mofl_graph_traits>;

  using vertices_type = std::map<VId, vertex_type>;
  using edges_type    = std::forward_list<edge_type>;
};

// Templated type alias for quick mofl_graph definition
template <class EV = void, class VV = void, class GV = void, class VId = uint32_t, bool Bidirectional = false>
using mofl_graph = dynamic_graph<EV, VV, GV, VId, Bidirectional, mofl_graph_traits<EV, VV, GV, VId, Bidirectional>>;
