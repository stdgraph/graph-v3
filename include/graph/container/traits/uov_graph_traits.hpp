...
//  Edges:    std::vector (contiguous; random access iterators)
...
//         Edges use std::vector for cache-friendly access and O(1) size().
...
struct uov_graph_traits {
  ...
  using edge_type   = dynamic_out_edge<EV, VV, GV, VId, Sourced, Bidirectional, uov_graph_traits>;
  using vertex_type = dynamic_vertex<EV, VV, GV, VId, Sourced, Bidirectional, uov_graph_traits>;
  using graph_type  = dynamic_graph<EV, VV, GV, VId, Sourced, Bidirectional, uov_graph_traits>;
