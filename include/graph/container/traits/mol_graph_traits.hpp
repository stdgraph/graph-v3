...
//         The std::list edge container provides bidirectional iteration (unlike forward_list).
...
struct mol_graph_traits {
  ...
  using edge_type   = dynamic_out_edge<EV, VV, GV, VId, Bidirectional, mol_graph_traits>;
  using vertex_type = dynamic_vertex<EV, VV, GV, VId, Bidirectional, mol_graph_traits>;
  using graph_type  = dynamic_graph<EV, VV, GV, VId, Bidirectional, mol_graph_traits>;

  using vertices_type = std::map<VId, vertex_type>;
  using edges_type    = std::list<edge_type>;
};

// Templated type alias for quick mol_graph definition
template <class EV = void, class VV = void, class GV = void, class VId = uint32_t, bool Bidirectional = false>
using mol_graph = dynamic_graph<EV, VV, GV, VId, Bidirectional, mol_graph_traits<EV, VV, GV, VId, Bidirectional>>;
