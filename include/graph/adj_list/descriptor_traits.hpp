 ...
 */
template <typename T>
concept random_access_descriptor = descriptor_type<T> && is_random_access_descriptor_v<std::remove_cvref_t<T>>;

/**
 * @brief Concept for iterator-based descriptors
 */
template <typename T>
concept iterator_based_descriptor = descriptor_type<T> && is_iterator_based_descriptor_v<std::remove_cvref_t<T>>;
