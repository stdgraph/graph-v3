 ...
 *
 * Key components:
 * - vertex_map<G, T>: Type alias — vector<T> for index graphs, unordered_map<VId, T> for mapped
 * - make_vertex_map: Factory functions (eager and lazy initialization)
 * - vertex_map_contains: Test whether a vertex ID has an entry
 * - vertex_map_get: Read with default fallback (no insertion for unordered_map)
 */
...
/**
 * @brief Eager initialization: create a vertex_map with every vertex pre-populated.
 *
 ...
 * @return A vertex_map<G, T> with all vertices pre-populated
 */
template <class G, class T>
...
/**
 * @brief Lazy initialization: create an empty vertex_map with capacity hint.
 *
 ...
 * @tparam T Value type
 * @param g The graph
 * @return A vertex_map<G, T> — sized for index graphs, empty+reserved for mapped
 */
template <class G, class T>
...
namespace graph {
using adj_list::vertex_map;
using adj_list::make_vertex_map;
using adj_list::vertex_map_contains;
using adj_list::vertex_map_get;
} // namespace graph
