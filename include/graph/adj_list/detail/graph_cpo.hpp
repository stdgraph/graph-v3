...
namespace _cpo_impls {
  ...
  namespace _vertices {
    ...
    template <typename G>
    concept _has_inner_value_pattern = std::ranges::forward_range<G> && requires {
      typename std::ranges::iterator_t<G>;
    ...
    public:
      /**
             ...
             */
      template <typename G>
      [[nodiscard]] constexpr auto operator()(G&& g) const noexcept(_Choice<std::remove_cvref_t<G>>._No_throw)
    ...
    concept _has_member_pid = requires(G& g, const PId& pid) {
      { g.vertices(pid) } -> std::ranges::forward_range;
    };

    // Check for ADL vertices(g, pid)
    template <typename G, typename PId>
    concept _has_adl_pid = requires(G& g, const PId& pid) {
    ...
    [[nodiscard]] consteval _Choice_t<_St_pid> _Choose_pid() noexcept {
      if constexpr (_has_member_pid<G, PId>) {
        return {_St_pid::_member, noexcept(std::declval<G&>().vertices(std::declval<const PId&>()))};
      } else if constexpr (_has_adl_pid<G, PId>) {
        return {_St_pid::_adl, noexcept(vertices(std::declval<G&>(), std::declval<const PId&>()))};
...
namespace _cpo_impls {
  ...
  namespace _vertex_id {
    ...
    concept _has_adl_descriptor = is_vertex_descriptor_v<std::remove_cvref_t<U>> && requires(const G& g, const U& u) {
      { vertex_id(g, u) };
    };

    // Check if descriptor has vertex_id() member (default)
    template <typename U>
    concept _has_descriptor = is_vertex_descriptor_v<std::remove_cvref_t<U>> && requires(const U& u) {
      { u.vertex_id() };
    };
    ...
    [[nodiscard]] consteval _Choice_t<_St> _Choose() noexcept {
      if constexpr (_has_adl_descriptor<G, U>) {
        return {_St::_adl_descriptor, noexcept(vertex_id(std::declval<const G&>(), std::declval<const U&>()))};
      } else if constexpr (_has_descriptor<U>) {
        return {_St::_descriptor, noexcept(std::declval<const U&>().vertex_id())};
      } else {
        return {_St::_none, false};
      }
    }
    ...
    public:
      ...
      [[nodiscard]] constexpr auto operator()(const G& g, const U& u) const
            noexcept(_Choice<std::remove_cvref_t<G>, std::remove_cvref_t<U>>._No_throw) -> decltype(auto)
...
namespace _cpo_impls {
  ...
  namespace _find_vertex {
    ...
    template <typename G, typename VId>
    concept _has_random_access = std::ranges::sized_range<decltype(vertices(std::declval<G&>()))> && requires(VId uid) {
    ...
    [[nodiscard]] consteval _Choice_t<_St> _Choose() noexcept {
      if constexpr (_has_member<G, VId>) {
        ...
        constexpr bool no_throw_next =
              noexcept(std::ranges::next(std::ranges::begin(vertices(std::declval<G&>())), std::declval<const VId&>()));
    ...
    public:
      ...
      [[nodiscard]] constexpr auto operator()(G&& g, const VId& uid) const
            noexcept(_Choice<std::remove_cvref_t<G>, std::remove_cvref_t<VId>>._No_throw) -> decltype(auto)
...
namespace _cpo_impls {
  ...
  namespace _edges {
    ...
    // Check for edges() member function on the underlying vertex object
    template <typename G, typename U>
    concept _has_vertex_member_u = is_vertex_descriptor_v<std::remove_cvref_t<U>> && std::ranges::forward_range<G> &&
                                   requires(G& g, const U& u) {
                                     { u.inner_value(g) }; // Check if inner_value exists and is valid
    ...
    template <typename G, typename U>
    concept _has_edge_value_pattern = is_vertex_descriptor_v<std::remove_cvref_t<U>> && requires(G& g, const U& u) {
    ...
    [[nodiscard]] consteval _Choice_t<_St_u> _Choose_u() noexcept {
      if constexpr (_has_vertex_member_u<G, U>) {
        return {_St_u::_vertex_member, noexcept(std::declval<const U&>().inner_value(std::declval<G&>()).edges())};
    ...
    public:
      /**
             ...
             * 
             * Resolution order:
             * 1. If u.inner_value(g).edges() exists -> use it (wrap in descriptor view if needed)
...
namespace _cpo_impls {
  ...
  namespace _target {
    ...
    template <typename G, typename E>
    concept _has_default = is_edge_descriptor_v<std::remove_cvref_t<E>> && requires(G& g, const E& uv) {
      { target_id(g, uv) };
    ...
    [[nodiscard]] consteval _Choice_t<_St> _Choose() noexcept {
      if constexpr (_has_member<G, E>) {
        return {_St::_member, noexcept(std::declval<G&>().target(std::declval<const E&>()))};
      } else if constexpr (_has_adl<G, E>) {
        return {_St::_adl, noexcept(target(std::declval<G&>(), std::declval<const E&>()))};
    ...
    public:
      ...
      [[nodiscard]] constexpr auto operator()(G&& g, const E& uv) const
            noexcept(_Choice<std::remove_cvref_t<G>, std::remove_cvref_t<E>>._No_throw) -> decltype(auto)
  ...
  namespace _num_vertices {
    ...
    public:
      /**
             ...
             */
      template <typename G>
      [[nodiscard]] constexpr auto operator()(const G& g) const noexcept(_Choice<std::remove_cvref_t<G>>._No_throw)
      requires(_Choice<std::remove_cvref_t<G>>._Strategy != _St::_none)
      {
        using _G = std::remove_cvref_t<G>;
    ...
    template <typename G, typename PId>
    concept _has_member_pid = requires(const G& g, const PId& pid) {
      { g.num_vertices(pid) } -> std::integral;
    };

    // Check for ADL num_vertices(g, pid)
    template <typename G, typename PId>
    concept _has_adl_pid = requires(const G& g, const PId& pid) {
    ...
    [[nodiscard]] consteval _Choice_t<_St_pid> _Choose_pid() noexcept {
      if constexpr (_has_member_pid<G, PId>) {
        return {_St_pid::_member, noexcept(std::declval<const G&>().num_vertices(std::declval<const PId&>()))};
      } else if constexpr (_has_adl_pid<G, PId>) {
        return {_St_pid::_adl, noexcept(num_vertices(std::declval<const G&>(), std::declval<const PId&>()))};
      } else if constexpr (_has_default_pid<G, PId>) {
        return {_St_pid::_default, true}; // Default is noexcept
      } else {
        return {_St_pid::_none, false};
    ...
    [[nodiscard]] consteval _Choice_t<_St_uu> _Choose_uu() noexcept {
      if constexpr (_has_member_uu<G, U, V>) {
        return {_St_uu::_member,
                noexcept(std::declval<G&>().find_vertex_edge(std::declval<const U&>(), std::declval<const V&>()))};
      } else if constexpr (_has_adl_uu<G, U, V>) {
        return {_St_uu::_adl,
                noexcept(find_vertex_edge(std::declval<G&>(), std::declval<const U&>(), std::declval<const V&>()))};
    ...
    [[nodiscard]] consteval _Choice_t<_St_uidvid> _Choose_uidvid() noexcept {
      if constexpr (_has_member_uidvid<G, UId, VId>) {
        return {_St_uidvid::_member,
                noexcept(std::declval<G&>().find_vertex_edge(std::declval<const UId&>(), std::declval<const VId&>()))};
      } else if constexpr (_has_adl_uidvid<G, UId, VId>) {
        return {_St_uidvid::_adl,
                noexcept(find_vertex_edge(std::declval<G&>(), std::declval<const UId&>(), std::declval<const VId&>()))};
    ...
    public:
        ...
        if constexpr (_Choice_uu<_G, _U, _V>._Strategy == _St_uu::_member) {
          ...
          auto it         = std::ranges::find_if(edge_range, [&](const auto& e) {
            return static_cast<vertex_id_t<_G>>(target_id(std::forward<G>(g), e)) ==
                   static_cast<vertex_id_t<_G>>(target_vid);
          });
                    ...
                    _St_uidvid::_none)
      {
        ...
        if constexpr (_Choice_uidvid<_G, _UId, _VId>._Strategy == _St_uidvid::_member) {
          if constexpr (requires { typename _G::vertex_id_type; }) {
            return g.find_vertex_edge(static_cast<typename _G::vertex_id_type>(uid),
          ...
          if constexpr (requires { typename _G::vertex_id_type; }) {
            return find_vertex_edge(g, static_cast<typename _G::vertex_id_type>(uid),
                                    static_cast<typename _G::vertex_id_type>(vid));
          } else {
...
namespace _cpo_impls {
  ...
  namespace _contains_edge {
    ...
    [[nodiscard]] consteval _Choice_t<_St_uv> _Choose_uv() noexcept {
      if constexpr (_has_member_uv<G, U, V>) {
        return {_St_uv::_member,
                noexcept(std::declval<G&>().contains_edge(std::declval<const U&>(), std::declval<const V&>()))};
      } else if constexpr (_has_adl_uv<G, U, V>) {
        return {_St_uv::_adl,
                noexcept(contains_edge(std::declval<G&>(), std::declval<const U&>(), std::declval<const V&>()))};
      } else if constexpr (_has_default_uv<G, U, V>) {
        return {_St_uv::_default, false};
      } else {
        return {_St_uv::_none, false};
    ...
    [[nodiscard]] consteval _Choice_t<_St_uidvid> _Choose_uidvid() noexcept {
      if constexpr (_has_member_uidvid<G, UId, VId>) {
        return {_St_uidvid::_member,
                noexcept(std::declval<G&>().contains_edge(std::declval<const UId&>(), std::declval<const VId&>()))};
      } else if constexpr (_has_adl_uidvid<G, UId, VId>) {
        return {_St_uidvid::_adl,
                noexcept(contains_edge(std::declval<G&>(), std::declval<const UId&>(), std::declval<const VId&>()))};
      } else if constexpr (_has_default_uidvid<G, UId, VId>) {
        return {_St_uidvid::_default, false};
      } else {
        return {_St_uidvid::_none, false};
    ...
    struct _fn {
                    ...
                    _St_uidvid::_none)
      {
        ...
        if constexpr (_Choice_uidvid<_G, _UId, _VId>._Strategy == _St_uidvid::_member) {
          if constexpr (requires { typename _G::vertex_id_type; }) {
            return g.contains_edge(static_cast<typename _G::vertex_id_type>(uid),
                                   static_cast<typename _G::vertex_id_type>(vid));
          } else {
          ...
          if constexpr (requires { typename _G::vertex_id_type; }) {
            return contains_edge(g, static_cast<typename _G::vertex_id_type>(uid),
                                 static_cast<typename _G::vertex_id_type>(vid));
          } else {
...
namespace _cpo_impls {
  ...
  namespace _has_edge {
    ...
    [[nodiscard]] consteval _Choice_t<_St> _Choose() noexcept {
      if constexpr (_has_member<G>) {
        return {_St::_member, noexcept(std::declval<const G&>().has_edge())};
      } else if constexpr (_has_adl<G>) {
        return {_St::_adl, noexcept(has_edge(std::declval<const G&>()))};
      } else if constexpr (_has_default<G>) {
        // Default implementation is not noexcept as it may allocate
        return {_St::_default, false};
      } else {
    ...
    public:
      /**
             ...
             */
      template <typename G>
      [[nodiscard]] constexpr bool operator()(G&& g) const noexcept(_Choice<std::remove_cvref_t<G>>._No_throw)
      requires(_Choice<std::remove_cvref_t<G>>._Strategy != _St::_none)
      {
        using _G = std::remove_cvref_t<G>;
...
namespace _cpo_impls {
  ...
  namespace _vertex_value {
    ...
    // Note: Uses G (not G&) to preserve const qualification
    template <typename G, typename U>
    concept _has_default = is_vertex_descriptor_v<std::remove_cvref_t<U>> && requires(G g, const U& u) {
      { u.inner_value(g) };
    };

    // Check if we can use the uid convenience overload:
    //   vertex_value(g, *find_vertex(g, uid))
    // Requires that uid is not itself a vertex descriptor, that find_vertex(g, uid)
    // is valid, and that vertex_value is available on the resulting vertex descriptor.
    template <typename G, typename VId>
    ...
    [[nodiscard]] consteval _Choice_t<_St> _Choose() noexcept {
      if constexpr (_has_member<G, U>) {
        return {_St::_member, noexcept(std::declval<G>().vertex_value(std::declval<const U&>()))};
      } else if constexpr (_has_adl<G, U>) {
        return {_St::_adl, noexcept(vertex_value(std::declval<G>(), std::declval<const U&>()))};
      } else if constexpr (_has_default<G, U>) {
        return {_St::_default, noexcept(std::declval<const U&>().inner_value(std::declval<G>()))};
      } else {
        return {_St::_none, false};
      }
    }
    ...
    public:
      ...
      /**
             ...
             *
             * Convenience overload that combines find_vertex + vertex_value(g, u):
             * resolves to vertex_value(g, *find_vertex(g, uid)). Complexity matches
             * find_vertex (O(1) for random-access, O(log n)/O(1) average for associative).
             *
             * @tparam G Graph type
             * @tparam VId Vertex ID type (typically vertex_id_t<G>)
             * @param g Graph container
             * @param uid Vertex ID
             * @return Exactly the type returned by the descriptor overload (decltype(auto)).
             */
      template <typename G, typename VId>
      ...
      [[nodiscard]] constexpr decltype(auto) operator()(G&& g, const VId& uid) const
            noexcept(_Choice<std::remove_cvref_t<G>, vertex_t<std::remove_cvref_t<G>>>._No_throw &&
...
namespace _cpo_impls {
  ...
  namespace _graph_value {
    ...
    [[nodiscard]] consteval _Choice_t<_St> _Choose() noexcept {
      if constexpr (_has_member<G>) {
        return {_St::_member, noexcept(std::declval<G>().graph_value())};
      } else if constexpr (_has_adl<G>) {
        return {_St::_adl, noexcept(graph_value(std::declval<G>()))};
      } else {
        return {_St::_none, false};
      }
    }
...
namespace _cpo_impls {
  ...
  namespace _source {
    ...
    template <typename E>
    concept _has_descriptor = is_edge_descriptor_v<std::remove_cvref_t<E>> && requires(const E& uv) {
      { uv.source() };
    };
    ...
    [[nodiscard]] consteval _Choice_t<_St> _Choose() noexcept {
      if constexpr (_has_member<G, E>) {
        return {_St::_member, noexcept(std::declval<G&>().source(std::declval<const E&>()))};
      } else if constexpr (_has_adl<G, E>) {
        return {_St::_adl, noexcept(source(std::declval<G&>(), std::declval<const E&>()))};
      } else if constexpr (_has_descriptor<E>) {
        return {_St::_descriptor, noexcept(std::declval<const E&>().source())};
      } else if constexpr (_has_default<G, E>) {
        return {_St::_default,
...
namespace _cpo_impls {
  ...
  namespace _partition_id {
    ...
    [[nodiscard]] consteval _Choice_t<_St> _Choose() noexcept {
      if constexpr (_has_member<G, U>) {
        return {_St::_member, noexcept(std::declval<G>().partition_id(std::declval<const U&>()))};
      } else if constexpr (_has_adl<G, U>) {
        return {_St::_adl, noexcept(partition_id(std::declval<G>(), std::declval<const U&>()))};
...
namespace _cpo_impls {
  ...
  namespace _num_partitions {
    ...
    [[nodiscard]] consteval _Choice_t<_St> _Choose() noexcept {
      if constexpr (_has_member<G>) {
        return {_St::_member, noexcept(std::declval<const G&>().num_partitions())};
      } else if constexpr (_has_adl<G>) {
        return {_St::_adl, noexcept(num_partitions(std::declval<const G&>()))};
      } else if constexpr (_has_default<G>) {
        return {_St::_default, true}; // Default is noexcept
      } else {
        return {_St::_none, false};
