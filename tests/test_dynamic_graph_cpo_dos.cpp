/**
 * @file test_dynamic_graph_cpo_dos.cpp
 * @brief Phase 4.1.3c CPO tests for dynamic_graph with dos_graph_traits
 * 
 * Tests CPO (Customization Point Object) integration with dynamic_graph.
 * These tests verify that CPOs work correctly with std::deque vertices + std::set edges.
 * 
 * Container: deque<vertex> + set<edge>
 * 
 ...
 * 
 * Key characteristics of dos_graph_traits:
...
using dos_void     = dynamic_graph<void, void, void, uint32_t, false, dos_graph_traits<void, void, void, uint32_t, false>>;
using dos_int_ev   = dynamic_graph<int, void, void, uint32_t, false, dos_graph_traits<int, void, void, uint32_t, false>>;
using dos_int_vv   = dynamic_graph<void, int, void, uint32_t, false, dos_graph_traits<void, int, void, uint32_t, false>>;
using dos_all_int  = dynamic_graph<int, int, int, uint32_t, false, dos_graph_traits<int, int, int, uint32_t, false>>;
using dos_string   = dynamic_graph<std::string, std::string, std::string, uint32_t, false, 
...
using dos_sourced_void = dynamic_graph<void, void, void, uint32_t, true, dos_graph_traits<void, void, void, uint32_t, true>>;
using dos_sourced_int  = dynamic_graph<int, void, void, uint32_t, true, dos_graph_traits<int, void, void, uint32_t, true>>;
using dos_sourced_all  = dynamic_graph<int, int, int, uint32_t, true, dos_graph_traits<int, int, int, uint32_t, true>>;
...
TEST_CASE("dos CPO vertices(g)", "[dynamic_graph][dos][cpo][vertices]") {
...
TEST_CASE("dos CPO num_vertices(g)", "[dynamic_graph][dos][cpo][num_vertices]") {
    SECTION("empty graph") {
        dos_void g;
...
TEST_CASE("dos CPO find_vertex(g, uid)", "[dynamic_graph][dos][cpo][find_vertex]") {
...
TEST_CASE("dos CPO vertex_id(g, u)", "[dynamic_graph][dos][cpo][vertex_id]") {
...
TEST_CASE("dos CPO num_edges(g)", "[dynamic_graph][dos][cpo][num_edges]") {
    SECTION("empty graph") {
        dos_void g;
...
// NOTE: num_edges(g, u) and num_edges(g, uid) NOT supported with dos_graph_traits
...
TEST_CASE("dos CPO edges(g, u)", "[dynamic_graph][dos][cpo][edges]") {
...
TEST_CASE("dos CPO edges(g, uid)", "[dynamic_graph][dos][cpo][edges]") {
    SECTION("basic iteration") {
        dos_void g({{0, 1}, {0, 2}});
...
TEST_CASE("dos CPO degree(g, u)", "[dynamic_graph][dos][cpo][degree]") {
    ...
    SECTION("multiple vertices") {
        dos_void g({{0, 1}, {0, 2}, {1, 2}, {2, 0}, {2, 1}});
...
TEST_CASE("dos CPO target_id(g, uv)", "[dynamic_graph][dos][cpo][target_id]") {
    SECTION("basic access") {
        dos_void g({{0, 5}});
...
TEST_CASE("dos CPO target(g, uv)", "[dynamic_graph][dos][cpo][target]") {
    SECTION("basic access") {
        dos_void g({{0, 1}});
...
TEST_CASE("dos CPO find_vertex_edge(g, u, v)", "[dynamic_graph][dos][cpo][find_vertex_edge]") {
...
TEST_CASE("dos CPO find_vertex_edge(g, uid, vid)", "[dynamic_graph][dos][cpo][find_vertex_edge][uid_vid]") {
    SECTION("existing edge") {
        dos_void g({{0, 1}, {0, 2}});
...
TEST_CASE("dos CPO contains_edge(g, u, v)", "[dynamic_graph][dos][cpo][contains_edge]") {
    SECTION("existing edge") {
        dos_void g({{0, 1}, {1, 2}});
...
TEST_CASE("dos CPO contains_edge(g, uid, vid)", "[dynamic_graph][dos][cpo][contains_edge][uid_vid]") {
    SECTION("existing edge") {
        dos_void g({{0, 1}, {1, 2}});
...
TEST_CASE("dos CPO has_edge(g)", "[dynamic_graph][dos][cpo][has_edge]") {
    SECTION("empty graph") {
        dos_void g;
...
TEST_CASE("dos CPO vertex_value(g, u)", "[dynamic_graph][dos][cpo][vertex_value]") {
    SECTION("read access") {
        dos_int_vv g;
...
TEST_CASE("dos CPO edge_value(g, uv)", "[dynamic_graph][dos][cpo][edge_value]") {
    SECTION("read access") {
        dos_int_ev g;
        std::vector<edge_int> ee = {{0, 1, 100}, {0, 2, 200}};
        g.load_edges(ee, std::identity{});
...
TEST_CASE("dos CPO graph_value(g)", "[dynamic_graph][dos][cpo][graph_value]") {
    SECTION("read access") {
        dos_all_int g(42);
    ...
    SECTION("const correctness") {
        const dos_all_int g(99);
        
        REQUIRE(graph_value(g) == 99);
    }

    SECTION("string value") {
        dos_string g(std::string("test"));
...
TEST_CASE("dos CPO partition_id(g, u)", "[dynamic_graph][dos][cpo][partition_id]") {
...
TEST_CASE("dos CPO num_partitions(g)", "[dynamic_graph][dos][cpo][num_partitions]") {
...
TEST_CASE("dos CPO vertices(g, pid)", "[dynamic_graph][dos][cpo][vertices][partition]") {
...
TEST_CASE("dos CPO num_vertices(g, pid)", "[dynamic_graph][dos][cpo][num_vertices][partition]") {
...
TEST_CASE("dos CPO source_id(g, uv)", "[dynamic_graph][dos][cpo][source_id]") {
    SECTION("basic access") {
        dos_sourced_void g({{0, 1}, {0, 2}, {1, 2}});
    ...
    SECTION("multiple sources") {
        dos_sourced_void g({{0, 2}, {1, 2}, {2, 0}});
...
TEST_CASE("dos CPO source(g, uv)", "[dynamic_graph][dos][cpo][source]") {
    SECTION("basic access") {
        dos_sourced_void g({{0, 1}, {1, 2}});
...
TEST_CASE("dos CPO integration", "[dynamic_graph][dos][cpo][integration]") {
    ...
    SECTION("graph traversal") {
        dos_void g({{0, 1}, {1, 2}, {2, 3}, {3, 0}});  // Cycle
...
TEST_CASE("dos CPO integration: modify vertex and edge values", "[dynamic_graph][dos][cpo][integration]") {
...
//==================================================================================================
// 28. Deque-Specific Tests
//==================================================================================================

TEST_CASE("dos CPO deque-specific: reference stability", "[dynamic_graph][dos][cpo][deque]") {
    SECTION("vertex references remain valid after resize") {
        dos_int_vv g;
        g.resize_vertices(5);
        ...
        // Resize (deque maintains reference stability on push_back)
        g.resize_vertices(100);
        
        // Verify original vertex values unchanged
        REQUIRE(vertex_value(g, *find_vertex(g, 0)) == 0);
        REQUIRE(vertex_value(g, *find_vertex(g, 1)) == 10);
        REQUIRE(vertex_value(g, *find_vertex(g, 2)) == 20);
        REQUIRE(vertex_value(g, *find_vertex(g, 3)) == 30);
        REQUIRE(vertex_value(g, *find_vertex(g, 4)) == 40);
    }
}
