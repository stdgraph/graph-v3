...
auto get(const EdgeWithSourceAndTarget& e) {
    if constexpr (I == 0) return e.source_id + 100;  // Different value to test precedence
    else return e.target_id + 100;
}
...
auto get(const EdgeWithAllThree& e) {
    ...
    else if constexpr (I == 1) return e.target_id;
    else return e.value + 100.0;  // Different value
}
