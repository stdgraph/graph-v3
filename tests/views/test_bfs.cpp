...
TEST_CASE("vertices_bfs - with value function", "[bfs][vertices]") {
    ...
    auto value_fn = [&g](auto v) { return static_cast<int>(vertex_id(g, v)) * 10; };
...
TEST_CASE("vertices_bfs - depth tracking", "[bfs][vertices]") {
    ...
    REQUIRE(bfs.size() == 3);   // 3 vertices visited after seed
}
...
TEST_CASE("vertices_bfs - size tracking", "[bfs][vertices]") {
    ...
    REQUIRE(bfs.size() == 2);  // 2 vertices visited after seed
}
...
TEST_CASE("vertices_bfs - depth increases by level", "[bfs][vertices][depth]") {
    ...
    for (auto [v] : bfs) {
        ...
        if (vid == 0) REQUIRE(current_depth == 0);      // Seed at level 0
        else if (vid <= 2) REQUIRE(current_depth >= 1); // Level 1
        else REQUIRE(current_depth >= 2);               // Level 2
...
TEST_CASE("vertices_bfs - size accumulates correctly", "[bfs][vertices][size]") {
    ...
    REQUIRE(bfs.size() == 5);  // 5 vertices visited after seed (excludes seed itself)
...
TEST_CASE("vertices_bfs - depth on wide tree", "[bfs][vertices][depth]") {
    ...
    REQUIRE(bfs.size() == 5);   // 5 children visited
}
...
TEST_CASE("vertices_bfs - depth on deep chain", "[bfs][vertices][depth]") {
    ...
    REQUIRE(bfs.size() == 5);   // 5 vertices after seed
}
...
TEST_CASE("vertices_bfs - size on disconnected graph", "[bfs][vertices][size]") {
    ...
    REQUIRE(bfs.size() == 2);   // Only 2 vertices reachable from 0 (excludes seed)
}
...
TEST_CASE("edges_bfs - depth tracks edge depth", "[bfs][edges][depth]") {
    ...
    REQUIRE(bfs.size() == 4);   // 4 edges total
}
...
TEST_CASE("edges_bfs - size counts edges", "[bfs][edges][size]") {
    ...
    for (auto [edge] : bfs) {
        ...
        REQUIRE(bfs.size() == static_cast<std::size_t>(edge_count));
    }
    
    REQUIRE(bfs.size() == 6);      // 6 edges total
...
TEST_CASE("vertices_bfs - depth/size with value function", "[bfs][vertices][depth]") {
    ...
    REQUIRE(bfs.size() == 3);   // 3 vertices after seed
}
...
TEST_CASE("edges_bfs - depth/size with value function", "[bfs][edges][depth]") {
    ...
    auto value_fn = [&g](auto e) { 
        return static_cast<int>(vertex_id(g, target(g, e))) * 10; 
    };
