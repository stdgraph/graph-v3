...
TEST_CASE("vertexlist adaptor - with value function", "[adaptors][vertexlist]") {
    ...
    auto vvf = [&g](auto&& v) { return vertex_id(g, v) * 10; };  // Simple transform
...
TEST_CASE("incidence adaptor - with value function", "[adaptors][incidence]") {
    ...
    auto evf = [&g](auto&& e) { return target_id(g, e) * 10; };  // Transform target ID
...
TEST_CASE("edgelist adaptor - with value function", "[adaptors][edgelist]") {
    ...
    auto evf = [&g](auto&& e) { return target_id(g, e) * 10; };
...
TEST_CASE("search adaptors - direct call compatibility", "[adaptors][dfs][bfs]") {
    ...
    for (auto [v] : dfs_view1) visited1.push_back(vertex_id(g, v));
    for (auto [v] : dfs_view2) visited2.push_back(vertex_id(g, v));
    ...
    visited1.clear();
    visited2.clear();
    for (auto [v] : bfs_view1) visited1.push_back(vertex_id(g, v));
    for (auto [v] : bfs_view2) visited2.push_back(vertex_id(g, v));
...
TEST_CASE("edges_topological_sort adaptor - with value function", "[adaptors][topological_sort]") {
    ...
    auto evf = [&g](auto e) { return vertex_id(g, source(g, e)) + vertex_id(g, target(g, e)); };
...
TEST_CASE("topological_sort adaptors - chaining with std::views", "[adaptors][topological_sort]") {
    ...
    std::vector<int> ids;
    for (auto id : g | vertices_topological_sort() 
                     | std::views::transform([&g](auto tup) { 
                         auto [v] = tup;
                         return vertex_id(g, v); 
                       })) {
        ids.push_back(id);
...
TEST_CASE("topological_sort adaptors - direct call compatibility", "[adaptors][topological_sort]") {
    ...
    std::vector<int> visited1, visited2;
    for (auto [v] : topo_view1) visited1.push_back(vertex_id(g, v));
    for (auto [v] : topo_view2) visited2.push_back(vertex_id(g, v));
