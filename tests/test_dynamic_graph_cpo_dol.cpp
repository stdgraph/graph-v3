/**
 * @file test_dynamic_graph_cpo_dol.cpp
 * @brief Phase 2 CPO tests for dynamic_graph with dol_graph_traits
 * 
 ...
 * 
 * Container: deque<vertex> + list<edge>
 * 
 ...
 *   NOTE: num_edges(g, u) and num_edges(g, uid) NOT supported with dol_graph_traits
 *   because list is not a sized_range. Use degree(g, u) instead for per-vertex counts.
 ...
 *   (provides equivalent functionality to num_edges(g, u) for dol)
 ...
 * 
 * Note: list uses push_back() for edge insertion, so edges appear in
 * insertion order (unlike forward_list which reverses order with push_front()).
 * 
 * Note: degree(g,u) uses the CPO default implementation with std::ranges::distance.
 * 
 * IMPORTANT LIMITATION: num_edges(g, u) and num_edges(g, uid) CPO overloads are NOT
 * supported with dol_graph_traits because:
 * - The default CPO implementation requires edges(g, u) to be a sized_range
 * - edges(g, u) returns edge_descriptor_view which only provides size() for random_access iterators
 * - std::list has bidirectional iterators (not random_access), so edge_descriptor_view is not a sized_range
 * 
 * WORKAROUND: Use degree(g, u) or degree(g, uid) instead, which provide equivalent
 * functionality for counting per-vertex edges. The degree() CPO uses std::ranges::distance
 * which works correctly with list and other bidirectional ranges.
 * 
 * To test num_edges(g, u), use vov_graph_traits which uses vector for edges (random_access iterator).
 */
...
using dol_void     = dynamic_graph<void, void, void, uint32_t, false, dol_graph_traits<void, void, void, uint32_t, false>>;
using dol_int_ev   = dynamic_graph<int, void, void, uint32_t, false, dol_graph_traits<int, void, void, uint32_t, false>>;
using dol_int_vv   = dynamic_graph<void, int, void, uint32_t, false, dol_graph_traits<void, int, void, uint32_t, false>>;
using dol_all_int  = dynamic_graph<int, int, int, uint32_t, false, dol_graph_traits<int, int, int, uint32_t, false>>;
using dol_string   = dynamic_graph<std::string, std::string, std::string, uint32_t, false, 
...
using dol_sourced_void = dynamic_graph<void, void, void, uint32_t, true, dol_graph_traits<void, void, void, uint32_t, true>>;
using dol_sourced_int  = dynamic_graph<int, void, void, uint32_t, true, dol_graph_traits<int, void, void, uint32_t, true>>;
using dol_sourced_all  = dynamic_graph<int, int, int, uint32_t, true, dol_graph_traits<int, int, int, uint32_t, true>>;

//==================================================================================================
// 1. vertices(g) CPO Tests
//==================================================================================================

TEST_CASE("dol CPO vertices(g)", "[dynamic_graph][dol][cpo][vertices]") {
...
TEST_CASE("dol CPO num_vertices(g)", "[dynamic_graph][dol][cpo][num_vertices]") {
    SECTION("empty graph") {
        dol_void g;
...
TEST_CASE("dol CPO find_vertex(g, uid)", "[dynamic_graph][dol][cpo][find_vertex]") {
...
TEST_CASE("dol CPO vertex_id(g, u)", "[dynamic_graph][dol][cpo][vertex_id]") {
...
TEST_CASE("dol CPO num_edges(g)", "[dynamic_graph][dol][cpo][num_edges]") {
    SECTION("empty graph") {
        dol_void g;
...
TEST_CASE("dol CPO edges(g, u)", "[dynamic_graph][dol][cpo][edges]") {
    ...
    SECTION("with edge values") {
        ...
        // list order: insertion order with push_back
...
TEST_CASE("dol CPO edges(g, uid)", "[dynamic_graph][dol][cpo][edges]") {
...
TEST_CASE("dol CPO degree(g, u)", "[dynamic_graph][dol][cpo][degree]") {
...
TEST_CASE("dol CPO target_id(g, uv)", "[dynamic_graph][dol][cpo][target_id]") {
    SECTION("basic access") {
        dol_void g({{0, 1}, {0, 2}, {1, 2}});
        ...
        auto uv0 = *it;
        REQUIRE(target_id(g, uv0) == 1);  // list: first added appears first
        
        ++it;
        REQUIRE(it != edge_view.end());
        auto uv1 = *it;
        REQUIRE(target_id(g, uv1) == 2);  // list: second added appears second
    }
    ...
    SECTION("with edge values") {
        dol_int_ev g({{0, 1, 100}, {0, 2, 200}, {1, 2, 300}});
    ...
    SECTION("self-loop") {
        dol_void g({{0, 0}, {0, 1}});  // Self-loop and regular edge
        
        auto u0 = *find_vertex(g, 0);
        auto edge_view = edges(g, u0);
        auto it = edge_view.begin();
        
        // list: first added (0,0) appears first - self-loop
    ...
    SECTION("with string edge values") {
        using dol_string_ev = dynamic_graph<std::string, void, void, uint32_t, false, 
    ...
    SECTION("iteration order") {
        // Verify target_id works correctly with list reverse insertion order
        std::vector<copyable_edge_t<uint32_t, void>> edge_data = {
            {0, 1}, {0, 2}, {0, 3}
        };
        dol_void g;
        ...
        // list uses push_back: edges appear in insertion order
...
TEST_CASE("dol CPO target(g, uv)", "[dynamic_graph][dol][cpo][target]") {
    SECTION("basic access") {
        dol_void g({{0, 1}, {0, 2}, {1, 2}});
        ...
        // Verify it's the correct vertex (list: first added appears first)
    ...
    SECTION("with string vertex values") {
        ...
        // Should have 2 targets (insertion order due to list)
...
TEST_CASE("dol CPO find_vertex_edge(g, u, v)", "[dynamic_graph][dol][cpo][find_vertex_edge]") {
    ...
    SECTION("with self-loop") {
        dol_void g({{0, 0}, {0, 1}});  // 0->0 (self-loop), 0->1
...
TEST_CASE("dol CPO find_vertex_edge(g, uid, vid)", "[dynamic_graph][dol][cpo][find_vertex_edge][uid_vid]") {
    SECTION("basic usage") {
        dol_void g({{0, 1}, {0, 2}, {1, 2}, {2, 3}});
...
TEST_CASE("dol CPO contains_edge(g, u, v)", "[dynamic_graph][dol][cpo][contains_edge]") {
    SECTION("edge exists") {
        dol_void g({{0, 1}, {0, 2}, {1, 2}});
...
TEST_CASE("dol CPO contains_edge(g, uid, vid)", "[dynamic_graph][dol][cpo][contains_edge][uid_vid]") {
    SECTION("basic usage") {
        dol_void g({{0, 1}, {0, 2}, {1, 2}, {2, 3}});
...
TEST_CASE("dol CPO integration", "[dynamic_graph][dol][cpo][integration]") {
...
TEST_CASE("dol CPO has_edge(g)", "[dynamic_graph][dol][cpo][has_edge]") {
    SECTION("empty graph") {
        dol_void g;
...
TEST_CASE("dol CPO vertex_value(g, u)", "[dynamic_graph][dol][cpo][vertex_value]") {
    SECTION("basic access") {
        dol_int_vv g;
        g.resize_vertices(3);
...
TEST_CASE("dol CPO edge_value(g, uv)", "[dynamic_graph][dol][cpo][edge_value]") {
    SECTION("basic access") {
        dol_int_ev g({{0, 1, 42}, {1, 2, 99}});
    ...
    SECTION("multiple edges") {
        ...
        // Note: list uses push_back, so edges are in insertion order of loading
        auto u = *vertices(g).begin();
        auto& v = u.inner_value(g);
        auto& edge_range = v.edges();
        auto e_iter = edge_range.begin();
...
TEST_CASE("dol CPO integration: values", "[dynamic_graph][dol][cpo][integration]") {
...
TEST_CASE("dol CPO graph_value(g)", "[dynamic_graph][dol][cpo][graph_value]") {
    SECTION("basic access") {
        dol_all_int g({{0, 1, 1}});
...
TEST_CASE("dol CPO partition_id(g, u)", "[dynamic_graph][dol][cpo][partition_id]") {
    ...
    SECTION("with different graph types") {
        dol_int_ev g1({{0, 1, 10}, {1, 2, 20}});
        dol_all_int g2({{0, 1, 1}, {1, 2, 2}});
        dol_string g3({{0, 1, "edge"}});
...
TEST_CASE("dol CPO num_partitions(g)", "[dynamic_graph][dol][cpo][num_partitions]") {
    ...
    SECTION("const correctness") {
        const dol_void g({{0, 1}});
        
        REQUIRE(num_partitions(g) == 1);
    }

    SECTION("consistency with partition_id") {
        dol_all_int g({{0, 1, 1}, {1, 2, 2}, {2, 3, 3}});
...
TEST_CASE("dol CPO vertices(g, pid)", "[dynamic_graph][dol][cpo][vertices][partition]") {
...
TEST_CASE("dol CPO num_vertices(g, pid)", "[dynamic_graph][dol][cpo][num_vertices][partition]") {
...
TEST_CASE("dol CPO source_id(g, uv)", "[dynamic_graph][dol][cpo][source_id]") {
    SECTION("basic usage") {
        dol_sourced_void g({{0, 1}, {1, 2}, {0, 2}});
    ...
    SECTION("multiple edges from same source") {
        dol_sourced_void g({{0, 1}, {0, 2}, {0, 3}});
    ...
    SECTION("different sources") {
        dol_sourced_void g({{0, 1}, {1, 2}, {2, 3}});
    ...
    SECTION("with edge values") {
        dol_sourced_int g({{0, 1, 10}, {1, 2, 20}, {2, 0, 30}});
    ...
    SECTION("const correctness") {
        const dol_sourced_void g({{0, 1}, {1, 2}});
    ...
    SECTION("parallel edges") {
        dol_sourced_int g({{0, 1, 10}, {0, 1, 20}, {0, 1, 30}});
    ...
    SECTION("star graph") {
        dol_sourced_void g({{0, 1}, {0, 2}, {0, 3}, {0, 4}});
    ...
    SECTION("chain graph") {
        dol_sourced_void g({{0, 1}, {1, 2}, {2, 3}, {3, 4}});
    ...
    SECTION("cycle graph") {
        dol_sourced_void g({{0, 1}, {1, 2}, {2, 3}, {3, 0}});
    ...
    SECTION("with all value types") {
        dol_sourced_all g({{0, 1, 100}, {1, 2, 200}, {2, 0, 300}});
...
TEST_CASE("dol CPO source(g, uv)", "[dynamic_graph][dol][cpo][source]") {
    SECTION("basic usage") {
        dol_sourced_void g({{0, 1}, {1, 2}, {0, 2}});
    ...
    SECTION("returns valid descriptor") {
        dol_sourced_void g({{0, 1}, {1, 2}, {2, 0}});
    ...
    SECTION("with vertex values") {
        dol_sourced_all g({{0, 1, 100}, {1, 2, 200}});
    ...
    SECTION("self-loops") {
        dol_sourced_void g({{0, 0}, {1, 1}, {2, 2}});
    ...
    SECTION("accumulate values from edges") {
        dol_sourced_all g({{0, 1, 100}, {1, 2, 200}, {2, 3, 300}});
...
TEST_CASE("dol CPO integration: modify vertex and edge values", "[dynamic_graph][dol][cpo][integration]") {
    dol_all_int g({{0, 1, 1}, {1, 2, 2}});
