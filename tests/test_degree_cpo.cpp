...
namespace test_adl {
    ...
    inline size_t degree(const GraphWithADLDegree& g, size_t u) {
        return g.adj_list[u].size() * 3;
    }

    inline size_t degree(GraphWithADLDegree& g, size_t u) {
        return g.adj_list[u].size() * 3;
    }
}
...
TEST_CASE("degree(g, u) works with weighted edges (tuple)", "[degree][cpo][weighted][tuple]") {
    ...
    std::vector<std::vector<Edge>> graph = {
        {{1, 1.5, "road"}, {2, 2.5, "rail"}, {3, 3.5, "air"}},
...
TEST_CASE("degree works with star graph", "[degree][cpo][topology][star]") {
    std::vector<std::vector<int>> graph = {
        {1, 2, 3, 4},  // Center (0) connected to all others
