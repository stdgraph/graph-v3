...
TEST_CASE("vertex_value - const graph", "[vertex_value][const]") {
  GraphWithVertexData        g_mutable = {{"Alice", 10}, {"Bob", 20}};
...
struct GraphWithConstOverloads {
  ...
  int& vertex_value(const vertex_descriptor<std::vector<int>::iterator>& u) { return data[u.value()]; }

  // Const version returns const reference
  const int& vertex_value(const vertex_descriptor<std::vector<int>::iterator>& u) const { return data[u.value()]; }
...
TEST_CASE("vertex_value - vector of pairs", "[vertex_value][types]") {
  std::vector<std::pair<int, std::string>> g = {{1, "one"}, {2, "two"}, {3, "three"}};
...
TEST_CASE("vertex_value - complex nested structures", "[vertex_value][complex]") {
  std::vector<ComplexVertex> g = {{"v0", {1, 2, 3}, {{"weight", 10.5}}},
...
TEST_CASE("vertex_value - typical BFS/DFS usage pattern", "[vertex_value][pattern]") {
  ...
  std::vector<VertexState> g = {{Color::White, -1, {1, 2}}, {Color::White, -1, {0, 2}}, {Color::White, -1, {0, 1}}};
