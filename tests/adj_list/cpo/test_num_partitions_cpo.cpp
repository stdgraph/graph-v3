...
TEST_CASE("num_partitions(g) - map with non-contiguous keys", "[num_partitions][cpo][default]") {
  using Graph = std::map<int, std::vector<int>>;
  Graph graph = {{10, {20, 30}}, {20, {30}}, {30, {10}}, {100, {10, 20}}};
...
TEST_CASE("num_partitions(g) - weighted graph with pairs", "[num_partitions][cpo][default]") {
  ...
  Graph graph = {{{1, 1.5}, {2, 2.5}}, {{2, 3.5}, {3, 4.5}}, {{3, 5.5}}};
...
TEST_CASE("num_partitions(g) - weighted graph with tuples", "[num_partitions][cpo][default]") {
  ...
  Graph graph = {{{1, 1.5, "a"}, {2, 2.5, "b"}}, {{2, 3.5, "c"}}, {{}}};
...
TEST_CASE("num_partitions(g) - relationship between num_vertices and num_partitions",
          "[num_partitions][cpo][default]") {
  using Graph = std::vector<std::vector<int>>;
  Graph graph = {{1, 2}, {2, 3}, {3}, {0}};
