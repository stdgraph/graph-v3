...
struct CustomGraphWithMember {
  ...
  auto find_vertex(std::size_t uid) {
    ...
    return std::ranges::next(std::ranges::begin(verts),
                             static_cast<std::iter_difference_t<decltype(std::ranges::begin(verts))>>(uid));
  }
};
...
TEST_CASE("find_vertex - map with weighted edges", "[graph_cpo][find_vertex][map]") {
  std::map<int, std::vector<std::pair<int, double>>> g = {{0, {{1, 1.5}, {2, 2.5}}}, {1, {{2, 3.5}}}, {2, {}}};
...
TEST_CASE("find_vertex - integration with vertices and vertex_id", "[graph_cpo][find_vertex][integration]") {
  ...
  SECTION("Round-trip: vertices -> vertex_id -> find_vertex") {
    ...
    for (auto v : verts) {
      ...
      REQUIRE(found ==
              std::ranges::next(std::ranges::begin(verts),
                                static_cast<std::iter_difference_t<decltype(std::ranges::begin(verts))>>(vid)));
  ...
  SECTION("find_vertex matches direct vertex access") {
    for (std::size_t i = 0; i < g.size(); ++i) {
      ...
      auto verts  = vertices(g);
      auto direct = std::ranges::next(std::ranges::begin(verts),
                                      static_cast<std::iter_difference_t<decltype(std::ranges::begin(verts))>>(i));
