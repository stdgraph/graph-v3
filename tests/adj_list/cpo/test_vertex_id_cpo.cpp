...
TEST_CASE("vertex_id(g,u) - vector<vector<int>> index-based IDs", "[vertex_id][cpo]") {
  ...
  SECTION("vertex_id_t is size_t for vector") { STATIC_REQUIRE(std::same_as<vertex_id_t<Graph>, size_t>); }
}
...
TEST_CASE("vertex_id(g,u) - map<int, vector<int>> key-based IDs", "[vertex_id][cpo]") {
  ...
  SECTION("vertex_id_t is int for map<int, ...>") { STATIC_REQUIRE(std::same_as<vertex_id_t<Graph>, int>); }
}
...
TEST_CASE("vertex_id(g,u) - map<string, vector<string>>", "[vertex_id][cpo]") {
  ...
  Graph graph = {{"Alice", {"Bob", "Charlie"}}, {"Bob", {"Charlie"}}, {"Charlie", {}}};
  ...
  SECTION("vertex_id_t is std::string") { STATIC_REQUIRE(std::same_as<vertex_id_t<Graph>, std::string>); }
}
...
TEST_CASE("vertex_id(g,u) - deque<deque<int>>", "[vertex_id][cpo]") {
  ...
  SECTION("vertex_id_t is size_t for deque") { STATIC_REQUIRE(std::same_as<vertex_id_t<Graph>, size_t>); }
}
...
TEST_CASE("vertex_id(g,u) - sparse map with non-contiguous keys", "[vertex_id][cpo]") {
  using Graph = std::map<int, std::vector<int>>;
  Graph graph = {{100, {200}}, {200, {300}}, {300, {400}}, {400, {}}};
...
int vertex_id(const GraphType& g, vertex_descriptor<GraphType::iterator> v) { return v.inner_value(g).value * 2; }
} // namespace test_adl_inner
