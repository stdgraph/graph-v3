...
TEST_CASE("edges(g,uid) - deque graph default", "[edges][cpo][uid][deque]") {
  std::deque<std::deque<int>> graph = {{10, 20}, {30}, {}, {10, 30}};
...
TEST_CASE("edges(g,uid) - map graph default", "[edges][cpo][uid][map]") {
  std::map<int, std::vector<int>> graph = {{100, {200, 300}}, {200, {300}}, {300, {}}, {400, {100, 200}}};
...
TEST_CASE("edges(g,uid) - weighted graph with pairs", "[edges][cpo][uid][weighted]") {
  std::vector<std::vector<std::pair<int, double>>> graph = {{{1, 1.5}, {2, 2.5}}, {{2, 3.5}}, {}};
...
TEST_CASE("edges(g,uid) - weighted graph with tuples", "[edges][cpo][uid][tuple]") {
  std::vector<std::vector<std::tuple<int, double, std::string>>> graph = {{{1, 1.5, "a"}, {2, 2.5, "b"}},
                                                                          {{0, 0.5, "c"}}};
...
auto edges(CustomGraph& g, std::size_t uid) {
  ...
  auto v = *std::ranges::next(std::ranges::begin(g.vertices()),
                              static_cast<std::iter_difference_t<decltype(std::ranges::begin(g.vertices()))>>(uid));
...
TEST_CASE("edges(g,uid) - integration with vertex_id and target", "[edges][cpo][uid][integration]") {
  std::vector<std::vector<int>> graph = {{1, 2}, {2, 3}, {3}, {0}};
...
TEST_CASE("edges(g,uid) - map with sparse IDs", "[edges][cpo][uid][sparse]") {
  std::map<int, std::vector<int>> graph = {{10, {50, 100}}, {50, {100}}, {100, {10}}};
...
TEST_CASE("edges(g,uid) - works with const graph", "[edges][cpo][uid][const]") {
  const std::vector<std::vector<int>> graph = {{1, 2}, {2, 3}, {3}, {}};
...
TEST_CASE("edges(g,uid) - complete graph K4", "[edges][cpo][uid][complete]") {
  std::vector<std::vector<int>> k4 = {{1, 2, 3}, {0, 2, 3}, {0, 1, 3}, {0, 1, 2}};
