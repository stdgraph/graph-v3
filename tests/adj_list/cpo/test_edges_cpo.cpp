...
TEST_CASE("edges(g,u) - vertex with edges() member function", "[edges][cpo][vertex_member]") {
  ...
  struct Vertex {
    ...
    const auto& edges() const { return edge_list; }
  };
...
namespace custom_ns {
struct CustomGraph {
  std::vector<std::vector<int>> data = {{10, 20}, {20}, {}};
};
...
TEST_CASE("edges(g,u) - range properties", "[edges][cpo][range]") {
  ...
  SECTION("Forward range") { REQUIRE(std::ranges::forward_range<decltype(edge_range)>); }
...
TEST_CASE("edges(g,u) - edge descriptor functionality", "[edges][cpo][descriptor]") {
  std::vector<std::vector<std::pair<int, double>>> graph = {{{1, 1.5}, {2, 2.5}}, {}};
