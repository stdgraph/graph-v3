/**
 * @file test_dynamic_graph_cpo_dofl.cpp
 * @brief Phase 2 CPO tests for dynamic_graph with dofl_graph_traits
 * 
 ...
 * 
 * Container: deque<vertex> + forward_list<edge>
 * 
 ...
 *   NOTE: num_edges(g, u) and num_edges(g, uid) NOT supported with dofl_graph_traits
 *   because forward_list is not a sized_range. Use degree(g, u) instead for per-vertex counts.
 ...
 *   (provides equivalent functionality to num_edges(g, u) for dofl)
 ...
 * supported with dofl_graph_traits because:
 * - The default CPO implementation requires edges(g, u) to be a sized_range
 * - std::forward_list is NOT a sized_range (doesn't provide O(1) size())
 * - forward_list deliberately omits size() for performance/space reasons
 * 
 ...
 * which works correctly with forward_list and other forward ranges.
 * 
 * To test num_edges(g, u), use vov_graph_traits or vol_graph_traits which use
 * vector or list for edges (both are sized ranges).
 */
...
using dofl_void     = dynamic_graph<void, void, void, uint32_t, false, dofl_graph_traits<void, void, void, uint32_t, false>>;
using dofl_int_ev   = dynamic_graph<int, void, void, uint32_t, false, dofl_graph_traits<int, void, void, uint32_t, false>>;
using dofl_int_vv   = dynamic_graph<void, int, void, uint32_t, false, dofl_graph_traits<void, int, void, uint32_t, false>>;
using dofl_all_int  = dynamic_graph<int, int, int, uint32_t, false, dofl_graph_traits<int, int, int, uint32_t, false>>;
using dofl_string   = dynamic_graph<std::string, std::string, std::string, uint32_t, false, 
                                     dofl_graph_traits<std::string, std::string, std::string, uint32_t, false>>;

// Type aliases for Sourced=true configurations (for source_id/source CPO tests)
using dofl_sourced_void = dynamic_graph<void, void, void, uint32_t, true, dofl_graph_traits<void, void, void, uint32_t, true>>;
using dofl_sourced_int  = dynamic_graph<int, void, void, uint32_t, true, dofl_graph_traits<int, void, void, uint32_t, true>>;
using dofl_sourced_all  = dynamic_graph<int, int, int, uint32_t, true, dofl_graph_traits<int, int, int, uint32_t, true>>;

//==================================================================================================
// 1. vertices(g) CPO Tests
//==================================================================================================

TEST_CASE("dofl CPO vertices(g)", "[dynamic_graph][dofl][cpo][vertices]") {
...
TEST_CASE("dofl CPO num_vertices(g)", "[dynamic_graph][dofl][cpo][num_vertices]") {
    SECTION("empty graph") {
        dofl_void g;
...
TEST_CASE("dofl CPO find_vertex(g, uid)", "[dynamic_graph][dofl][cpo][find_vertex]") {
...
TEST_CASE("dofl CPO vertex_id(g, u)", "[dynamic_graph][dofl][cpo][vertex_id]") {
...
TEST_CASE("dofl CPO num_edges(g)", "[dynamic_graph][dofl][cpo][num_edges]") {
    SECTION("empty graph") {
        dofl_void g;
...
TEST_CASE("dofl CPO edges(g, u)", "[dynamic_graph][dofl][cpo][edges]") {
...
TEST_CASE("dofl CPO edges(g, uid)", "[dynamic_graph][dofl][cpo][edges]") {
...
TEST_CASE("dofl CPO degree(g, u)", "[dynamic_graph][dofl][cpo][degree]") {
...
TEST_CASE("dofl CPO target_id(g, uv)", "[dynamic_graph][dofl][cpo][target_id]") {
    SECTION("basic access") {
        dofl_void g({{0, 1}, {0, 2}, {1, 2}});
    ...
    SECTION("with edge values") {
        dofl_int_ev g({{0, 1, 100}, {0, 2, 200}, {1, 2, 300}});
    ...
    SECTION("self-loop") {
        dofl_void g({{0, 0}, {0, 1}});  // Self-loop and regular edge
    ...
    SECTION("with string edge values") {
        using dofl_string_ev = dynamic_graph<std::string, void, void, uint32_t, false, 
                                             dofl_graph_traits<std::string, void, void, uint32_t, false>>;
...
TEST_CASE("dofl CPO target(g, uv)", "[dynamic_graph][dofl][cpo][target]") {
    SECTION("basic access") {
        dofl_void g({{0, 1}, {0, 2}, {1, 2}});
...
TEST_CASE("dofl CPO find_vertex_edge(g, u, v)", "[dynamic_graph][dofl][cpo][find_vertex_edge]") {
    ...
    SECTION("with self-loop") {
        dofl_void g({{0, 0}, {0, 1}});  // 0->0 (self-loop), 0->1
...
TEST_CASE("dofl CPO find_vertex_edge(g, uid, vid)", "[dynamic_graph][dofl][cpo][find_vertex_edge][uid_vid]") {
    SECTION("basic usage") {
        dofl_void g({{0, 1}, {0, 2}, {1, 2}, {2, 3}});
...
TEST_CASE("dofl CPO contains_edge(g, u, v)", "[dynamic_graph][dofl][cpo][contains_edge]") {
    SECTION("edge exists") {
        dofl_void g({{0, 1}, {0, 2}, {1, 2}});
...
TEST_CASE("dofl CPO contains_edge(g, uid, vid)", "[dynamic_graph][dofl][cpo][contains_edge][uid_vid]") {
    SECTION("basic usage") {
        dofl_void g({{0, 1}, {0, 2}, {1, 2}, {2, 3}});
...
TEST_CASE("dofl CPO integration", "[dynamic_graph][dofl][cpo][integration]") {
...
TEST_CASE("dofl CPO has_edge(g)", "[dynamic_graph][dofl][cpo][has_edge]") {
    SECTION("empty graph") {
        dofl_void g;
...
TEST_CASE("dofl CPO vertex_value(g, u)", "[dynamic_graph][dofl][cpo][vertex_value]") {
    SECTION("basic access") {
        dofl_int_vv g;
        g.resize_vertices(3);
...
TEST_CASE("dofl CPO edge_value(g, uv)", "[dynamic_graph][dofl][cpo][edge_value]") {
    SECTION("basic access") {
        dofl_int_ev g({{0, 1, 42}, {1, 2, 99}});
    ...
    SECTION("const correctness") {
        dofl_int_ev g({{0, 1, 42}});
        
        const dofl_int_ev& const_g = g;
        auto const_u = *vertices(const_g).begin();
...
TEST_CASE("dofl CPO integration: values", "[dynamic_graph][dofl][cpo][integration]") {
...
TEST_CASE("dofl CPO graph_value(g)", "[dynamic_graph][dofl][cpo][graph_value]") {
    SECTION("basic access") {
        dofl_all_int g({{0, 1, 1}});
...
TEST_CASE("dofl CPO partition_id(g, u)", "[dynamic_graph][dofl][cpo][partition_id]") {
    ...
    SECTION("with different graph types") {
        dofl_int_ev g1({{0, 1, 10}, {1, 2, 20}});
        dofl_all_int g2({{0, 1, 1}, {1, 2, 2}});
        dofl_string g3({{0, 1, "edge"}});
...
TEST_CASE("dofl CPO num_partitions(g)", "[dynamic_graph][dofl][cpo][num_partitions]") {
    ...
    SECTION("const correctness") {
        const dofl_void g({{0, 1}});
        
        REQUIRE(num_partitions(g) == 1);
    }

    SECTION("consistency with partition_id") {
        dofl_all_int g({{0, 1, 1}, {1, 2, 2}, {2, 3, 3}});
...
TEST_CASE("dofl CPO vertices(g, pid)", "[dynamic_graph][dofl][cpo][vertices][partition]") {
...
TEST_CASE("dofl CPO num_vertices(g, pid)", "[dynamic_graph][dofl][cpo][num_vertices][partition]") {
...
TEST_CASE("dofl CPO source_id(g, uv)", "[dynamic_graph][dofl][cpo][source_id]") {
    SECTION("basic usage") {
        dofl_sourced_void g({{0, 1}, {1, 2}, {0, 2}});
    ...
    SECTION("multiple edges from same source") {
        dofl_sourced_void g({{0, 1}, {0, 2}, {0, 3}});
    ...
    SECTION("different sources") {
        dofl_sourced_void g({{0, 1}, {1, 2}, {2, 3}});
    ...
    SECTION("with edge values") {
        dofl_sourced_int g({{0, 1, 10}, {1, 2, 20}, {2, 0, 30}});
    ...
    SECTION("const correctness") {
        const dofl_sourced_void g({{0, 1}, {1, 2}});
    ...
    SECTION("parallel edges") {
        dofl_sourced_int g({{0, 1, 10}, {0, 1, 20}, {0, 1, 30}});
    ...
    SECTION("star graph") {
        dofl_sourced_void g({{0, 1}, {0, 2}, {0, 3}, {0, 4}});
    ...
    SECTION("chain graph") {
        dofl_sourced_void g({{0, 1}, {1, 2}, {2, 3}, {3, 4}});
    ...
    SECTION("cycle graph") {
        dofl_sourced_void g({{0, 1}, {1, 2}, {2, 3}, {3, 0}});
    ...
    SECTION("with all value types") {
        dofl_sourced_all g({{0, 1, 100}, {1, 2, 200}, {2, 0, 300}});
...
TEST_CASE("dofl CPO source(g, uv)", "[dynamic_graph][dofl][cpo][source]") {
    SECTION("basic usage") {
        dofl_sourced_void g({{0, 1}, {1, 2}, {0, 2}});
    ...
    SECTION("returns valid descriptor") {
        dofl_sourced_void g({{0, 1}, {1, 2}, {2, 0}});
    ...
    SECTION("with vertex values") {
        dofl_sourced_all g({{0, 1, 100}, {1, 2, 200}});
    ...
    SECTION("self-loops") {
        dofl_sourced_void g({{0, 0}, {1, 1}, {2, 2}});
    ...
    SECTION("accumulate values from edges") {
        dofl_sourced_all g({{0, 1, 100}, {1, 2, 200}, {2, 3, 300}});
...
TEST_CASE("dofl CPO integration: modify vertex and edge values", "[dynamic_graph][dofl][cpo][integration]") {
    dofl_all_int g({{0, 1, 1}, {1, 2, 2}});
