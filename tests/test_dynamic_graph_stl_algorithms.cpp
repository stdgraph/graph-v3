...
using vov_int_edges = dynamic_graph<int, void, void, uint64_t, false,
                                     vov_graph_traits<int, void, void, uint64_t, false>>;
using dofl_int_edges = dynamic_graph<int, void, void, uint64_t, false,
                                      dofl_graph_traits<int, void, void, uint64_t, false>>;

// Sequential container graphs (integral VId) - int vertices
using vov_int_verts = dynamic_graph<void, int, void, uint64_t, false,
                                     vov_graph_traits<void, int, void, uint64_t, false>>;
using dofl_int_verts = dynamic_graph<void, int, void, uint64_t, false,
                                      dofl_graph_traits<void, int, void, uint64_t, false>>;

// Sequential container graphs (integral VId) - int edges and vertices
using vov_int_both = dynamic_graph<int, int, void, uint64_t, false,
                                    vov_graph_traits<int, int, void, uint64_t, false>>;
using dofl_int_both = dynamic_graph<int, int, void, uint64_t, false,
...
using mos_int_edges = dynamic_graph<int, void, void, std::string, false,
                                     mos_graph_traits<int, void, void, std::string, false>>;

// Map-based graphs (string VId) - int vertices
using mos_int_verts = dynamic_graph<void, int, void, std::string, false,
...
TEST_CASE("for_each - sum edge values in vov graph", "[stl][6.2.1][for_each]") {
    ...
    int sum = 0;
    std::ranges::for_each(vertices(g), [&sum, &g](auto&& v) {
        std::ranges::for_each(edges(g, v), [&sum, &g](auto&& e) {
            sum += edge_value(g, e);
        });
    });
...
TEST_CASE("for_each - collect edge target IDs in vov graph", "[stl][6.2.1][for_each]") {
    ...
    std::ranges::for_each(vertices(g), [&targets, &g](auto&& v) {
        std::ranges::for_each(edges(g, v), [&targets, &g](auto&& e) {
...
TEST_CASE("for_each - modify edge values in place", "[stl][6.2.1][for_each]") {
    ...
    std::ranges::for_each(vertices(g), [&g](auto&& v) {
        std::ranges::for_each(edges(g, v), [&g](auto&& e) {
            edge_value(g, e) *= 3;
        });
    });
...
TEST_CASE("for_each - modify both vertex and edge values", "[stl][6.2.1][for_each]") {
    ...
    int vertex_sum = 0;
    std::ranges::for_each(vertices(g), [&vertex_sum, &g](auto&& v) {
        vertex_sum += vertex_value(g, v);
    });
    ...
    std::ranges::for_each(vertices(g), [&edge_sum, &g](auto&& v) {
        std::ranges::for_each(edges(g, v), [&edge_sum, &g](auto&& e) {
...
TEST_CASE("for_each - complex accumulation pattern", "[stl][6.2.1][for_each]") {
    ...
    std::ranges::for_each(vertices(g), [&total, &g](auto&& v) {
        ...
        std::ranges::for_each(edges(g, v), [&total, &g](auto&& e) {
            total += edge_value(g, e);
        });
    });
...
TEST_CASE("for_each - collect all vertex IDs in mos graph", "[stl][6.2.1][for_each]") {
    ...
    std::ranges::for_each(vertices(g), [&ids, &g](auto&& v) {
...
TEST_CASE("find_if - find vertex by ID in vov graph", "[stl][6.2.2][find_if]") {
    ...
    auto it = std::ranges::find_if(verts, [&g](auto&& v) {
...
TEST_CASE("find_if - find vertex by value predicate in mos graph", "[stl][6.2.2][find_if]") {
    ...
    auto verts = vertices(g);
    auto found = std::ranges::find_if(verts, [&g](auto&& v) {
        return vertex_value(g, v) > 150;
    });
...
TEST_CASE("find_if - find isolated vertex", "[stl][6.2.2][find_if]") {
    ...
    auto verts = vertices(g);
    auto it = std::ranges::find_if(verts, [&g](auto&& v) {
        return std::ranges::distance(edges(g, v)) == 0;
    });
...
TEST_CASE("find_if_not - find vertex without edges", "[stl][6.2.2][find_if_not]") {
    ...
    auto verts = vertices(g);
    auto it = std::ranges::find_if_not(verts, [&g](auto&& v) {
        return std::ranges::distance(edges(g, v)) > 0;
    });
...
TEST_CASE("any_of - check if any vertex has high degree", "[stl][6.2.2][any_of]") {
    ...
    bool has_high_degree = std::ranges::any_of(vertices(g), [&g](auto&& v) {
        return std::ranges::distance(edges(g, v)) >= 3;
    });
...
TEST_CASE("any_of - check if any vertex has specific value", "[stl][6.2.2][any_of]") {
    ...
    bool has_value_10 = std::ranges::any_of(vertices(g), [&g](auto&& v) {
        return vertex_value(g, v) == 10;
    });
...
TEST_CASE("all_of - check if all vertices have edges", "[stl][6.2.2][all_of]") {
    ...
    bool all_have_edges = std::ranges::all_of(vertices(g), [&g](auto&& v) {
        return std::ranges::distance(edges(g, v)) > 0;
    });
...
TEST_CASE("all_of - check if all vertices have positive values", "[stl][6.2.2][all_of]") {
    ...
    bool all_positive = std::ranges::all_of(vertices(g), [&g](auto&& v) {
        return vertex_value(g, v) > 0;
    });
...
TEST_CASE("none_of - check if no vertex is isolated in connected graph", "[stl][6.2.2][none_of]") {
    ...
    bool none_isolated = std::ranges::none_of(vertices(g), [&g](auto&& v) {
        return std::ranges::distance(edges(g, v)) == 0;
    });
...
TEST_CASE("none_of - check if no vertex has negative value", "[stl][6.2.2][none_of]") {
    ...
    bool none_negative = std::ranges::none_of(vertices(g), [&g](auto&& v) {
        return vertex_value(g, v) < 0;
    });
...
TEST_CASE("count_if - count edges with specific target (vov)", "[stl][6.2.3][count_if]") {
    ...
    auto count = std::ranges::count_if(edges(g, v), [&g](auto&& e) {
        return target_id(g, e) == 1;
    });
...
TEST_CASE("count_if - count self-loops in specific vertex (mos)", "[stl][6.2.3][count_if]") {
    ...
    auto count = std::ranges::count_if(edges(g, v), [&g, &uid](auto&& e) {
        return target_id(g, e) == uid;
    });
...
TEST_CASE("count_if - count edges with both conditions (vov)", "[stl][6.2.3][count_if]") {
    ...
    auto count = std::ranges::count_if(edges(g, v), [&g](auto&& e) {
        return target_id(g, e) > 1 && edge_value(g, e) >= 20;
    });
...
TEST_CASE("count_if - count with complex predicate (vov)", "[stl][6.2.3][count_if]") {
    ...
    auto count = std::ranges::count_if(vertices(g), [&g](auto&& v) {
        return vertex_value(g, v) >= 10 && std::ranges::distance(edges(g, v)) > 0;
    });
...
TEST_CASE("count_if - count vertices in mos with string predicate", "[stl][6.2.3][count_if]") {
    ...
    for (auto&& v : vertices(g)) {
        auto id = vertex_id(g, v);
        if (id == "apple") vertex_value(g, v) = 10;
        else if (id == "banana") vertex_value(g, v) = 20;
        else if (id == "cherry") vertex_value(g, v) = 30;
        else if (id == "date") vertex_value(g, v) = 40;
    }
...
TEST_CASE("count_if - filter vertices then count (vov)", "[stl][6.2.3][count_if][filter]") {
    ...
    auto filtered = vertices(g) | std::views::filter([&g](auto&& v) {
        return vertex_value(g, v) >= 25;
    });
    
    auto count = std::ranges::count_if(filtered, [&g](auto&& v) {
        return vertex_value(g, v) <= 45;
    });
...
TEST_CASE("count_if - filter edges then count (dofl)", "[stl][6.2.3][count_if][filter]") {
    ...
    auto count = std::ranges::count_if(filtered, [&g](auto&& e) {
        return target_id(g, e) >= 2;
    });
...
TEST_CASE("count_if - count using views::transform (vov)", "[stl][6.2.3][count_if][transform]") {
    ...
    auto targets = edges(g, v) | std::views::transform([&g](auto&& e) {
        return target_id(g, e);
    });
    
    auto count = std::ranges::count_if(targets, [](auto tid) {
        return tid >= 5;
    });
...
TEST_CASE("transform - extract vertex IDs to vector (vov)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(vertices(g), std::back_inserter(ids), [&g](auto&& v) {
...
TEST_CASE("transform - extract edge target IDs to vector (vov)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(edges(g, v), std::back_inserter(targets), [&g](auto&& e) {
        return target_id(g, e);
    });
...
TEST_CASE("transform - double vertex values (vov)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(vertices(g), std::back_inserter(doubled), [&g](auto&& v) {
...
TEST_CASE("transform - increment edge values (dofl)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(edges(g, v), std::back_inserter(incremented), [&g](auto&& e) {
        return edge_value(g, e) + 1;
    });
...
TEST_CASE("transform - vertex IDs to strings (vov)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(vertices(g), std::back_inserter(id_strings), [&g](auto&& v) {
...
TEST_CASE("transform - compute edge weights as double (vov)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(edges(g, v), std::back_inserter(weights), [&g](auto&& e) {
        return static_cast<double>(edge_value(g, e)) / 10.0;
    });
...
TEST_CASE("transform - filter then extract IDs (vov)", "[stl][6.2.4][transform][filter]") {
    ...
    auto v = *vertices(g).begin();
    auto filtered = edges(g, v) | std::views::filter([&g](auto&& e) {
        return target_id(g, e) % 2 == 0;
    });
    
    std::vector<size_t> even_targets;
    std::ranges::transform(filtered, std::back_inserter(even_targets), [&g](auto&& e) {
        return target_id(g, e);
    });
...
TEST_CASE("transform - vertex degree to vector (dofl)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(vertices(g), std::back_inserter(degrees), [&g](auto&& v) {
        return std::ranges::distance(edges(g, v));
    });
...
TEST_CASE("transform - combine vertex ID and value (vov)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(vertices(g), std::back_inserter(id_value_pairs), [&g](auto&& v) {
        return std::make_pair(vertex_id(g, v), vertex_value(g, v));
    });
...
TEST_CASE("transform - edge source and target pairs (vov)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(edges(g, v), std::back_inserter(edge_pairs), [&g, v_id = vertex_id(g, v)](auto&& e) {
        return std::make_pair(v_id, target_id(g, e));
    });
...
TEST_CASE("transform - negate edge values (dofl)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(edges(g, v), std::back_inserter(negated), [&g](auto&& e) {
...
TEST_CASE("transform - vertices to booleans (even IDs) (vov)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(vertices(g), std::back_inserter(is_even), [&g](auto&& v) {
        return vertex_id(g, v) % 2 == 0;
    });
...
TEST_CASE("transform - filter vertices then transform values (vov)", "[stl][6.2.4][transform][filter]") {
    ...
    std::vector<int> doubled;
    std::ranges::transform(filtered, std::back_inserter(doubled), [&g](auto&& v) {
        return vertex_value(g, v) * 2;
    });
...
TEST_CASE("transform - empty edge list (vov)", "[stl][6.2.4][transform]") {
    ...
    auto verts = vertices(g);
    auto it = verts.begin();
    ++it; ++it; // Move to vertex 2
    
    std::vector<size_t> targets;
    std::ranges::transform(edges(g, *it), std::back_inserter(targets), [&g](auto&& e) {
        return target_id(g, e);
    });
...
TEST_CASE("transform - compute vertex ID differences (vov)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(edges(g, v), std::back_inserter(differences), [&g, source_id](auto&& e) {
        return target_id(g, e) - source_id;
    });
...
TEST_CASE("transform - chain filter and transform with views (dofl)", "[stl][6.2.4][transform][filter]") {
    ...
    auto v = *vertices(g).begin();
    auto result = edges(g, v) 
        | std::views::filter([&g](auto&& e) { return edge_value(g, e) >= 10; })
...
TEST_CASE("transform - create edge descriptors with values (dofl)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(edges(g, v), std::back_inserter(edge_infos), [&g](auto&& e) {
        return EdgeInfo{target_id(g, e), edge_value(g, e)};
    });
...
TEST_CASE("transform - multiple transformations in sequence (vov)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(doubled, std::back_inserter(final), [](int val) {
        return val + 5;
    });
...
TEST_CASE("transform - filter edges by value range then transform (dofl)", "[stl][6.2.4][transform][filter]") {
    ...
    std::ranges::transform(filtered, std::back_inserter(results), [&g](auto&& e) {
        return std::make_pair(target_id(g, e), edge_value(g, e) + 100);
    });
...
TEST_CASE("transform - map vertices with complex struct (vov)", "[stl][6.2.4][transform]") {
    ...
    std::ranges::transform(vertices(g), std::back_inserter(vertex_data), [&g](auto&& v) {
        return VertexData{
            vertex_id(g, v),
            vertex_value(g, v),
            static_cast<size_t>(std::ranges::distance(edges(g, v)))
        };
    });
...
TEST_CASE("sort edge target IDs (vov)", "[stl][6.2.5][sort]") {
    ...
    std::ranges::transform(edge_range, std::back_inserter(targets), [&g](auto&& e) {
        return target_id(g, e);
    });
...
TEST_CASE("sort edge values (vov)", "[stl][6.2.5][sort]") {
    ...
    auto edge_range = edges(g, 0);
    std::vector<int> values;
    std::ranges::transform(edge_range, std::back_inserter(values), [&g](auto&& e) {
        return edge_value(g, e);
    });
...
TEST_CASE("sort vertex values (vov)", "[stl][6.2.5][sort]") {
    ...
    std::vector<int> values;
    std::ranges::transform(vertices(g), std::back_inserter(values), [&g](auto&& v) {
...
TEST_CASE("sort by vertex value with projection (vov)", "[stl][6.2.5][sort]") {
    ...
    std::ranges::transform(vertices(g), std::back_inserter(infos), [&g](auto&& v) {
        return VertexInfo{vertex_id(g, v), vertex_value(g, v)};
    });
...
TEST_CASE("sort edge infos by target then by value (vov)", "[stl][6.2.5][sort]") {
    ...
    std::ranges::transform(edge_range, std::back_inserter(infos), [&g](auto&& e) {
        return EdgeInfo{target_id(g, e), edge_value(g, e)};
    });
...
TEST_CASE("sort combined vertex ID and value pairs (vov)", "[stl][6.2.5][sort]") {
    ...
    std::ranges::transform(vertices(g), std::back_inserter(pairs), [&g](auto&& v) {
        return std::make_pair(vertex_id(g, v), vertex_value(g, v));
    });
...
TEST_CASE("stable_sort preserves relative order of equal elements (vov)", "[stl][6.2.5][sort]") {
    ...
    size_t order = 0;
    std::ranges::transform(edge_range, std::back_inserter(data), [&g, &order](auto&& e) {
        return EdgeData{order++, target_id(g, e), edge_value(g, e)};
    });
...
TEST_CASE("sort with custom comparator (vov)", "[stl][6.2.5][sort]") {
    ...
    std::ranges::sort(values, [](int a, int b) {
...
TEST_CASE("sort with both vertex and edge values (vov)", "[stl][6.2.5][sort]") {
    ...
    std::ranges::sort(data, [](const auto& a, const auto& b) {
        return a.vval < b.vval || (a.vval == b.vval && a.eval < b.eval);
    });
