...
using vos_void = dynamic_graph<void, void, void, uint64_t, false,
                                vos_graph_traits<void, void, void, uint64_t, false>>;

// Unordered set-based edge container graphs (integral VId) - void edges
using vous_void = dynamic_graph<void, void, void, uint64_t, false,
                                 vous_graph_traits<void, void, void, uint64_t, false>>;

// Set-based edge container graphs (integral VId) - int edges
using vos_int = dynamic_graph<int, void, void, uint64_t, false,
                               vos_graph_traits<int, void, void, uint64_t, false>>;

// String edge value types
using vov_string = dynamic_graph<std::string, void, void, uint64_t, false,
                                  vov_graph_traits<std::string, void, void, uint64_t, false>>;
using vofl_string = dynamic_graph<std::string, void, void, uint64_t, false,
                                   vofl_graph_traits<std::string, void, void, uint64_t, false>>;
using mos_string = dynamic_graph<std::string, void, void, std::string, false,
                                  mos_graph_traits<std::string, void, void, std::string, false>>;
using mol_string = dynamic_graph<std::string, void, void, std::string, false,
...
TEST_CASE("Self-loop - find_edge finds self-loop", "[integration][6.5.3][self_loop]") {
    ...
    auto edge_rng = edges(g, 0);
    auto it = std::ranges::find_if(edge_rng, [&](auto&& e) {
        return target_id(g, e) == 0;
    });
    ...
    for (auto&& v : vertices(source)) {
        for (auto&& e : edges(source, v)) {
            edge_list.push_back({
                vertex_id(source, v),
                target_id(source, e),
                std::to_string(edge_value(source, e))
            });
        }
    }
    ...
    for (auto&& v : vertices(source)) {
        for (auto&& e : edges(source, v)) {
            edge_list.push_back({
                vertex_id(source, v),
                target_id(source, e),
                "value_" + std::to_string(edge_value(source, e))
            });
        }
    }
    ...
    for (auto&& v : vertices(source)) {
        for (auto&& e : edges(source, v)) {
            edge_list.push_back({
                vertex_id(source, v),
                target_id(source, e),
                "lucky_" + std::to_string(edge_value(source, e))
            });
        }
    }
...
TEST_CASE("Value conversion - chain transformations", "[integration][6.5.5][conversion]") {
    ...
    for (auto&& v : vertices(g1)) {
        for (auto&& e : edges(g1, v)) {
            edge_list1.push_back({
                vertex_id(g1, v),
                target_id(g1, e),
                std::to_string(edge_value(g1, e))
            });
        }
    }
    ...
    for (auto&& v : vertices(g2)) {
        for (auto&& e : edges(g2, v)) {
            edge_list2.push_back({
                vertex_id(g2, v),
                target_id(g2, e),
                std::stoi(edge_value(g2, e)) * 2
            });
        }
    }
    ...
    for (auto&& v : vertices(source)) {
        for (auto&& e : edges(source, v)) {
            edge_list.push_back({
                vertex_id(source, v),
                target_id(source, e),
                "val:" + std::to_string(edge_value(source, e))
            });
        }
    }
...
TEST_CASE("Real-world - citation network", "[integration][6.5.6][real_world]") {
    ...
    mos_int citations({
        {"Paper_A", "Paper_B", 2020},
        {"Paper_A", "Paper_C", 2021},
        {"Paper_B", "Paper_D", 2022}
    });
...
TEST_CASE("Real-world - gene regulatory network", "[integration][6.5.6][real_world]") {
    ...
    mos_int gene_network({
        {"GeneA", "GeneB", 1},   // activates
...
TEST_CASE("Real-world - supply chain", "[integration][6.5.6][real_world]") {
    ...
    mos_int supply_chain({
        {"Supplier", "Warehouse", 3},
        {"Warehouse", "Store1", 1},
        {"Warehouse", "Store2", 1},
        {"Warehouse", "Store3", 2}
    });
