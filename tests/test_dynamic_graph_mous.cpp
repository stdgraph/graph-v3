/**
 * @file test_dynamic_graph_mous.cpp
 * @brief Tests for dynamic_graph with map vertices + unordered_set edges
 * 
 * Phase 4.2.4: Map Vertex + Unordered Set Edge Containers
 * Tests mous_graph_traits (map vertices + unordered_set edges)
 * 
 * Key characteristics:
...
using mous_void_void_void = dynamic_graph<void, void, void, uint32_t, false, mous_graph_traits<void, void, void, uint32_t, false>>;
using mous_int_void_void = dynamic_graph<int, void, void, uint32_t, false, mous_graph_traits<int, void, void, uint32_t, false>>;
using mous_void_int_void = dynamic_graph<void, int, void, uint32_t, false, mous_graph_traits<void, int, void, uint32_t, false>>;
using mous_int_int_void = dynamic_graph<int, int, void, uint32_t, false, mous_graph_traits<int, int, void, uint32_t, false>>;
using mous_void_void_int = dynamic_graph<void, void, int, uint32_t, false, mous_graph_traits<void, void, int, uint32_t, false>>;
using mous_int_int_int = dynamic_graph<int, int, int, uint32_t, false, mous_graph_traits<int, int, int, uint32_t, false>>;

// Type aliases with string vertex IDs (the primary use case for map containers)
using mous_str_void_void_void = dynamic_graph<void, void, void, std::string, false, mous_graph_traits<void, void, void, std::string, false>>;
using mous_str_int_void_void = dynamic_graph<int, void, void, std::string, false, mous_graph_traits<int, void, void, std::string, false>>;
using mous_str_void_int_void = dynamic_graph<void, int, void, std::string, false, mous_graph_traits<void, int, void, std::string, false>>;
using mous_str_int_int_int = dynamic_graph<int, int, int, std::string, false, mous_graph_traits<int, int, int, std::string, false>>;

using mous_sourced = dynamic_graph<void, void, void, uint32_t, true, mous_graph_traits<void, void, void, uint32_t, true>>;
using mous_int_sourced = dynamic_graph<int, void, void, uint32_t, true, mous_graph_traits<int, void, void, uint32_t, true>>;

using mous_str_sourced = dynamic_graph<void, void, void, std::string, true, mous_graph_traits<void, void, void, std::string, true>>;
...
TEST_CASE("mous traits verification", "[dynamic_graph][mous][traits]") {
    ...
    SECTION("vertex_id_type can be string") {
        using traits = mous_graph_traits<void, void, void, std::string, false>;
    ...
    SECTION("custom vertex_id_type") {
        using traits = mous_graph_traits<void, void, void, int64_t, false>;
...
TEST_CASE("mous iterator categories", "[dynamic_graph][mous][iterators]") {
    SECTION("underlying map iterators are bidirectional") {
        using G = mous_void_void_void;
    ...
    SECTION("graph is a range") {
        static_assert(std::ranges::range<mous_void_void_void>);
        static_assert(std::ranges::range<mous_int_int_int>);
        static_assert(std::ranges::range<mous_str_void_void_void>);
        REQUIRE(true);
    }
}
...
TEST_CASE("mous construction", "[dynamic_graph][mous][construction]") {
    SECTION("default constructor creates empty graph") {
        mous_void_void_void g;
        REQUIRE(g.size() == 0);
    }
    ...
    SECTION("copy constructor") {
        mous_int_int_int g1;
        mous_int_int_int g2(g1);
        REQUIRE(g2.size() == g1.size());
    }

    SECTION("move constructor") {
        mous_int_int_int g1;
        mous_int_int_int g2(std::move(g1));
        REQUIRE(g2.size() == 0);
    }

    SECTION("copy assignment") {
        mous_int_int_int g1, g2;
        g2 = g1;
        REQUIRE(g2.size() == g1.size());
    }
...
TEST_CASE("mous construction with string vertex IDs", "[dynamic_graph][mous][construction][string]") {
    SECTION("default constructor creates empty graph") {
        mous_str_void_void_void g;
        REQUIRE(g.size() == 0);
    }

    SECTION("default constructor with int edge values") {
        mous_str_int_void_void g;
        REQUIRE(g.size() == 0);
    }

    SECTION("default constructor with int vertex values") {
        mous_str_void_int_void g;
        REQUIRE(g.size() == 0);
    }

    SECTION("default constructor with all int values") {
        mous_str_int_int_int g;
        REQUIRE(g.size() == 0);
    }
}
...
TEST_CASE("mous properties", "[dynamic_graph][mous][properties]") {
...
TEST_CASE("mous properties with string IDs", "[dynamic_graph][mous][properties][string]") {
    SECTION("size() on empty graph") {
        mous_str_void_void_void g;
        REQUIRE(g.size() == 0);
    }
...
TEST_CASE("mous initializer_list construction", "[dynamic_graph][mous][initializer_list]") {
    ...
    SECTION("sparse vertex IDs - only referenced vertices created") {
        ...
        G g({{100, 200}});
        REQUIRE(g.size() == 2);
        // With mous, only vertices 100 and 200 are created
    }
...
TEST_CASE("mous edge deduplication", "[dynamic_graph][mous][unordered_set][deduplication]") {
    ...
    SECTION("parallel edges NOT allowed - unlike mofl") {
        mous_int_void_void g({{0, 1, 1}, {0, 1, 2}, {0, 1, 3}});
        REQUIRE(g.size() == 2);
...
TEST_CASE("mous edges are unordered by target_id", "[dynamic_graph][mous][unordered_set][unordered]") {
...
TEST_CASE("mous initializer_list construction string IDs", "[dynamic_graph][mous][initializer_list][string]") {
    ...
    SECTION("string IDs with edge values") {
        using G = mous_str_int_void_void;
...
TEST_CASE("mous graph value access", "[dynamic_graph][mous][graph_value]") {
    SECTION("graph_value() returns reference") {
        mous_void_void_int g(100);
    ...
    SECTION("const graph_value()") {
        const mous_void_void_int g(42);
        REQUIRE(g.graph_value() == 42);
    }

    SECTION("graph value preserved through copy") {
        mous_void_void_int g1(100);
        mous_void_void_int g2 = g1;
    ...
    SECTION("graph value preserved through move") {
        mous_void_void_int g1(100);
        mous_void_void_int g2 = std::move(g1);
        REQUIRE(g2.graph_value() == 100);
    }
}
...
TEST_CASE("mous graph iteration", "[dynamic_graph][mous][iteration]") {
    ...
    SECTION("vertices in sorted key order") {
        mous_void_void_void g({{5, 1}, {3, 2}, {7, 4}, {1, 6}});
        ...
        // Map guarantees sorted order by key
...
TEST_CASE("mous contains_vertex", "[dynamic_graph][mous][accessor][contains_vertex]") {
...
TEST_CASE("mous try_find_vertex", "[dynamic_graph][mous][accessor][try_find_vertex]") {
...
TEST_CASE("mous vertex_at", "[dynamic_graph][mous][accessor][vertex_at]") {
    SECTION("uint32_t vertex IDs - found") {
        using G = mous_void_void_void;
        G g({{0, 1}, {1, 2}});
    ...
    SECTION("modify vertex through vertex_at") {
        using G = mous_void_int_void;  // has vertex value
        G g({{0, 1}});
...
TEST_CASE("mous load_vertices", "[dynamic_graph][mous][load_vertices]") {
    SECTION("uint32_t IDs - basic load") {
        using G = mous_void_int_void;
    ...
    SECTION("string IDs - basic load") {
        using G = mous_str_void_int_void;
...
TEST_CASE("mous load_edges explicit", "[dynamic_graph][mous][load_edges]") {
    SECTION("uint32_t IDs - basic load") {
        using G = mous_int_void_void;
...
TEST_CASE("mous edge cases", "[dynamic_graph][mous][edge_cases]") {
...
TEST_CASE("mous const correctness", "[dynamic_graph][mous][const]") {
...
TEST_CASE("mous memory management", "[dynamic_graph][mous][memory]") {
...
TEST_CASE("mous template instantiation", "[dynamic_graph][mous][compilation]") {
    [[maybe_unused]] mous_void_void_void g1;
    [[maybe_unused]] mous_int_void_void g2;
    [[maybe_unused]] mous_void_int_void g3;
...
TEST_CASE("mous sparse vertex behavior", "[dynamic_graph][mous][sparse]") {
...
TEST_CASE("mous edge bidirectional iteration", "[dynamic_graph][mous][unordered_set][iteration]") {
    SECTION("forward iteration") {
        mous_void_void_void g({{0, 1}, {0, 2}, {0, 3}});
