...
// Type alias for vertex id to avoid conversion warnings
using VKey = unsigned int;

// Helper to convert iterator difference to vertex id
...
TEST_CASE("create single vertex", "[undirected_adjacency_list][vertex][create]") {
  ...
  SECTION("vertex has no edges") { REQUIRE(v_it->num_edges() == 0); }
}
...
TEST_CASE("create multiple vertices", "[undirected_adjacency_list][vertex][create]") {
  ...
  SECTION("graph has three vertices") { REQUIRE(g.vertices().size() == 3); }
  ...
  SECTION("vertices have sequential keys") {
    REQUIRE(vertex_id(v1, g) == 0);
    REQUIRE(vertex_id(v2, g) == 1);
    REQUIRE(vertex_id(v3, g) == 2);
  }
}
...
TEST_CASE("create single edge", "[undirected_adjacency_list][edge][create]") {
  ...
  SECTION("edge has correct value") { REQUIRE(e_it->value() == 100); }

  SECTION("edge connects correct vertices") {
    REQUIRE(e_it->list_owner_id() == k1);
    REQUIRE(e_it->list_target_id() == k2);
  }
...
TEST_CASE("remove edge", "[undirected_adjacency_list][edge][erase]") {
  ...
  SECTION("graph has two edges") { REQUIRE(g.num_edges() == 2); }
...
TEST_CASE("modify edge value", "[undirected_adjacency_list][edge][modify]") {
  ...
  SECTION("edge value updated via iterator") { REQUIRE(e->value() == 999); }
...
TEST_CASE("std::find with edge_iterator", "[undirected_adjacency_list][iterators][algorithms]") {
  ...
  auto it = std::find_if(range.begin(), range.end(), [](const auto& e) { return e.value() == 300; });
...
TEST_CASE("self-loop erasure", "[undirected_adjacency_list][edge_cases][self_loop]") {
  ...
  SECTION("erase one self-loop leaves the other") {
    ...
    auto  it    = std::find_if(edges.begin(), edges.end(), [](const auto& e) { return e.value() == 100; });
...
TEST_CASE("parallel edges", "[undirected_adjacency_list][edge_cases][parallel]") {
  ...
  SECTION("erasing one parallel edge leaves others") {
    ...
    auto  it    = std::find_if(edges.begin(), edges.end(), [](const auto& e) { return e.value() == 200; });
...
TEST_CASE("edge erasure consistency", "[undirected_adjacency_list][edge_cases][erase][consistency]") {
  ...
  SECTION("removed from graph count") { REQUIRE(g.num_edges() == 0); }
...
TEST_CASE("copy constructor", "[undirected_adjacency_list][memory][copy]") {
  ...
  SECTION("copy has same vertex count") { REQUIRE(g2.vertices().size() == 2); }
  ...
  SECTION("copy has same edge count") { REQUIRE(g2.num_edges() == g1.num_edges()); }
...
TEST_CASE("edge range constructor basic", "[undirected_adjacency_list][construction][range]") {
  ...
  undirected_adjacency_list<int, int, int> g(edge_list, std::identity{},
                                             42 // graph value
  );
  ...
  SECTION("correct number of edges created") { REQUIRE(g.num_edges() == 3); }

  SECTION("graph value set correctly") { REQUIRE(g.graph_value() == 42); }
...
TEST_CASE("edge range constructor sparse vertices", "[undirected_adjacency_list][construction][range]") {
  ...
  undirected_adjacency_list<int, int, int> g(edge_list, std::identity{}, 0);
