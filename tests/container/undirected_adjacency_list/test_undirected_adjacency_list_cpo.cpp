...
TEST_CASE("graph_value CPO", "[undirected_adjacency_list][cpo][graph_value]") {
  ...
  SECTION("read graph value") { REQUIRE(graph_value(g) == "my graph"); }
...
TEST_CASE("CPO with empty graph", "[undirected_adjacency_list][cpo][empty]") {
  ...
  SECTION("num_vertices on empty graph") { REQUIRE(num_vertices(g) == 0); }
  ...
  SECTION("graph_value on empty graph") { REQUIRE(graph_value(g) == 42); }
}
