...
using vofl_void_void_void =
      vofl_graph<void, void, void>;
using vofl_int_void_void =
      vofl_graph<int, void, void>;
using vofl_void_int_void =
      vofl_graph<void, int, void>;
using vofl_int_int_void =
      vofl_graph<int, int, void>;
using vofl_void_void_int =
      vofl_graph<void, void, int>;
using vofl_int_int_int =
      vofl_graph<int, int, int>;
...
using vofl_string_string_string =
      dynamic_graph<std::string,
                    ...
                    std::string,
                    uint32_t,
                    false, vofl_graph_traits<std::string, std::string, std::string, uint32_t, false>>;
...
TEST_CASE("vofl value_types", "[dynamic_graph][vofl][value_types]") {
  SECTION("with void edge value") {
    using graph_t = vofl_graph<void, int, int>;
    graph_t g(100);
    REQUIRE(g.graph_value() == 100);
  }

  SECTION("with void vertex value") {
    using graph_t = vofl_graph<int, void, int>;
    graph_t g(100);
    REQUIRE(g.graph_value() == 100);
  }

  SECTION("with void graph value") {
    using graph_t = vofl_graph<int, int, void>;
    graph_t g;
    REQUIRE(g.size() == 0);
  }
  ...
  SECTION("with string edge value type") {
    using graph_t = vofl_graph<std::string, void, void>;
    graph_t g;
    REQUIRE(g.size() == 0);
  }

  SECTION("with string vertex value type") {
    using graph_t = vofl_graph<void, std::string, void>;
    graph_t g;
    REQUIRE(g.size() == 0);
  }

  SECTION("with string graph value type") {
    using graph_t = vofl_graph<void, void, std::string>;
...
TEST_CASE("vofl vertex_id", "[dynamic_graph][vofl][vertex_id]") {
  ...
  SECTION("with int32_t vertex id") {
    using graph_t =
          vofl_graph<void, void, void, int32_t>;
    graph_t g;
    REQUIRE(g.size() == 0);
  }

  SECTION("with int8_t vertex id") {
    using graph_t = vofl_graph<void, void, void, int8_t>;
    graph_t g;
    REQUIRE(g.size() == 0);
  }

  SECTION("with int vertex id") {
    using graph_t = vofl_graph<void, void, void, int>;
    graph_t g;
    REQUIRE(g.size() == 0);
  }
}
...
TEST_CASE("vofl load_vertices", "[dynamic_graph][vofl][load_vertices]") {
  ...
  SECTION("custom projection - load with projection from struct") {
    using G           = vofl_graph<int, std::string, void>;
...
TEST_CASE("vofl load_edges", "[dynamic_graph][vofl][load_edges]") {
  ...
  SECTION("custom projection - load with projection from custom struct") {
    using G           = vofl_graph<std::string, int, void>;
    using vertex_data = copyable_vertex_t<uint32_t, int>;
...
TEST_CASE("vofl contains_vertex", "[dynamic_graph][vofl][accessor][contains_vertex]") {
  ...
  SECTION("sparse graph - all indices up to max exist") {
    using G = vofl_void_void_void;
    // Edge from 0 to 10 - creates vertices 0-10 (unlike map)
    G g({{0, 10}});
    REQUIRE(g.size() == 11);
...
TEST_CASE("vofl try_find_vertex", "[dynamic_graph][vofl][accessor][try_find_vertex]") {
  SECTION("found") {
    using G = vofl_void_void_void;
    G g({{0, 1}, {1, 2}});
...
TEST_CASE("vofl vertex_at", "[dynamic_graph][vofl][accessor][vertex_at]") {
  SECTION("found") {
    using G = vofl_void_void_void;
    G g({{0, 1}, {1, 2}});
  ...
  SECTION("modify vertex through vertex_at") {
    using G = vofl_void_int_void; // has vertex value
    G g({{0, 1}});
