/**
 * @file test_dynamic_graph_cpo_vod.cpp
 * @brief Phase 2 CPO tests for dynamic_graph with vod_graph_traits
 * 
 ...
 * 
 * Container: vector<vertex> + deque<edge>
 * 
 ...
 * - num_edges(g, u) - Get edge count for vertex (SUPPORTED with deque - random_access + sized_range)
 * - num_edges(g, uid) - Get edge count by vertex ID (SUPPORTED with deque - random_access + sized_range)
 ...
 *   (provides equivalent functionality to num_edges(g, u) for vod)
 ...
 * 
 * Note: deque uses push_back() for edge insertion, so edges appear in
 * insertion order (like vector and list).
 * 
 * Note: degree(g,u) uses the CPO default implementation with std::ranges::distance.
 * 
 * IMPORTANT: Like vov_graph_traits (vector with random_access iterators), vod_graph_traits
 * (deque with random_access iterators) DOES support num_edges(g, u) and num_edges(g, uid) 
 * CPO overloads because:
 * - edges(g, u) returns edge_descriptor_view which provides size() for random_access iterators
 * - std::deque has random_access iterators, so edge_descriptor_view IS a sized_range
 * - This allows O(1) edge counting per vertex via num_edges(g, u)
 * 
 * Key deque characteristics tested:
 * - Random access iterators (like vector)
 * - Stable iterators (unlike vector) - not invalidated on push_back/push_front
 * - Sized range with O(1) size() operation
 */
...
using vod_void     = dynamic_graph<void, void, void, uint32_t, false, vod_graph_traits<void, void, void, uint32_t, false>>;
using vod_int_ev   = dynamic_graph<int, void, void, uint32_t, false, vod_graph_traits<int, void, void, uint32_t, false>>;
using vod_int_vv   = dynamic_graph<void, int, void, uint32_t, false, vod_graph_traits<void, int, void, uint32_t, false>>;
using vod_all_int  = dynamic_graph<int, int, int, uint32_t, false, vod_graph_traits<int, int, int, uint32_t, false>>;
using vod_string   = dynamic_graph<std::string, std::string, std::string, uint32_t, false, 
...
using vod_sourced_void = dynamic_graph<void, void, void, uint32_t, true, vod_graph_traits<void, void, void, uint32_t, true>>;
using vod_sourced_int  = dynamic_graph<int, void, void, uint32_t, true, vod_graph_traits<int, void, void, uint32_t, true>>;
using vod_sourced_all  = dynamic_graph<int, int, int, uint32_t, true, vod_graph_traits<int, int, int, uint32_t, true>>;

//==================================================================================================
// 1. vertices(g) CPO Tests
//==================================================================================================

TEST_CASE("vod CPO vertices(g)", "[dynamic_graph][vod][cpo][vertices]") {
...
TEST_CASE("vod CPO num_vertices(g)", "[dynamic_graph][vod][cpo][num_vertices]") {
    SECTION("empty graph") {
        vod_void g;
...
TEST_CASE("vod CPO find_vertex(g, uid)", "[dynamic_graph][vod][cpo][find_vertex]") {
...
TEST_CASE("vod CPO vertex_id(g, u)", "[dynamic_graph][vod][cpo][vertex_id]") {
...
TEST_CASE("vod CPO num_edges(g)", "[dynamic_graph][vod][cpo][num_edges]") {
    SECTION("empty graph") {
        vod_void g;
...
TEST_CASE("vod CPO num_edges(g, u)", "[dynamic_graph][vod][cpo][num_edges]") {
...
TEST_CASE("vod CPO num_edges(g, uid)", "[dynamic_graph][vod][cpo][num_edges]") {
...
TEST_CASE("vod CPO edges(g, u)", "[dynamic_graph][vod][cpo][edges]") {
...
TEST_CASE("vod CPO edges(g, uid)", "[dynamic_graph][vod][cpo][edges]") {
...
TEST_CASE("vod CPO degree(g, u)", "[dynamic_graph][vod][cpo][degree]") {
...
TEST_CASE("vod CPO target_id(g, uv)", "[dynamic_graph][vod][cpo][target_id]") {
    SECTION("basic access") {
        vod_void g({{0, 1}, {0, 2}, {1, 2}});
    ...
    SECTION("with edge values") {
        vod_int_ev g({{0, 1, 100}, {0, 2, 200}, {1, 2, 300}});
    ...
    SECTION("self-loop") {
        vod_void g({{0, 0}, {0, 1}});  // Self-loop and regular edge
    ...
    SECTION("with string edge values") {
        using vod_string_ev = dynamic_graph<std::string, void, void, uint32_t, false, 
...
TEST_CASE("vod CPO target(g, uv)", "[dynamic_graph][vod][cpo][target]") {
    SECTION("basic access") {
        vod_void g({{0, 1}, {0, 2}, {1, 2}});
...
TEST_CASE("vod CPO find_vertex_edge(g, u, v)", "[dynamic_graph][vod][cpo][find_vertex_edge]") {
    ...
    SECTION("with self-loop") {
        vod_void g({{0, 0}, {0, 1}});  // 0->0 (self-loop), 0->1
...
TEST_CASE("vod CPO find_vertex_edge(g, uid, vid)", "[dynamic_graph][vod][cpo][find_vertex_edge][uid_vid]") {
    SECTION("basic usage") {
        vod_void g({{0, 1}, {0, 2}, {1, 2}, {2, 3}});
...
TEST_CASE("vod CPO contains_edge(g, u, v)", "[dynamic_graph][vod][cpo][contains_edge]") {
    SECTION("edge exists") {
        vod_void g({{0, 1}, {0, 2}, {1, 2}});
...
TEST_CASE("vod CPO contains_edge(g, uid, vid)", "[dynamic_graph][vod][cpo][contains_edge][uid_vid]") {
    SECTION("basic usage") {
        vod_void g({{0, 1}, {0, 2}, {1, 2}, {2, 3}});
...
TEST_CASE("vod CPO integration", "[dynamic_graph][vod][cpo][integration]") {
...
TEST_CASE("vod CPO has_edge(g)", "[dynamic_graph][vod][cpo][has_edge]") {
    SECTION("empty graph") {
        vod_void g;
...
TEST_CASE("vod CPO vertex_value(g, u)", "[dynamic_graph][vod][cpo][vertex_value]") {
    SECTION("basic access") {
        vod_int_vv g;
        g.resize_vertices(3);
...
TEST_CASE("vod CPO edge_value(g, uv)", "[dynamic_graph][vod][cpo][edge_value]") {
    SECTION("basic access") {
        vod_int_ev g({{0, 1, 42}, {1, 2, 99}});
...
TEST_CASE("vod CPO integration: values", "[dynamic_graph][vod][cpo][integration]") {
...
TEST_CASE("vod CPO graph_value(g)", "[dynamic_graph][vod][cpo][graph_value]") {
    SECTION("basic access") {
        vod_all_int g({{0, 1, 1}});
...
TEST_CASE("vod CPO partition_id(g, u)", "[dynamic_graph][vod][cpo][partition_id]") {
    ...
    SECTION("with different graph types") {
        vod_int_ev g1({{0, 1, 10}, {1, 2, 20}});
        vod_all_int g2({{0, 1, 1}, {1, 2, 2}});
        vod_string g3({{0, 1, "edge"}});
...
TEST_CASE("vod CPO num_partitions(g)", "[dynamic_graph][vod][cpo][num_partitions]") {
    ...
    SECTION("const correctness") {
        const vod_void g({{0, 1}});
        
        REQUIRE(num_partitions(g) == 1);
    }

    SECTION("consistency with partition_id") {
        vod_all_int g({{0, 1, 1}, {1, 2, 2}, {2, 3, 3}});
...
TEST_CASE("vod CPO vertices(g, pid)", "[dynamic_graph][vod][cpo][vertices][partition]") {
...
TEST_CASE("vod CPO num_vertices(g, pid)", "[dynamic_graph][vod][cpo][num_vertices][partition]") {
...
TEST_CASE("vod CPO source_id(g, uv)", "[dynamic_graph][vod][cpo][source_id]") {
    SECTION("basic usage") {
        vod_sourced_void g({{0, 1}, {1, 2}, {0, 2}});
    ...
    SECTION("multiple edges from same source") {
        vod_sourced_void g({{0, 1}, {0, 2}, {0, 3}});
    ...
    SECTION("different sources") {
        vod_sourced_void g({{0, 1}, {1, 2}, {2, 3}});
    ...
    SECTION("with edge values") {
        vod_sourced_int g({{0, 1, 10}, {1, 2, 20}, {2, 0, 30}});
    ...
    SECTION("const correctness") {
        const vod_sourced_void g({{0, 1}, {1, 2}});
    ...
    SECTION("parallel edges") {
        vod_sourced_int g({{0, 1, 10}, {0, 1, 20}, {0, 1, 30}});
    ...
    SECTION("star graph") {
        vod_sourced_void g({{0, 1}, {0, 2}, {0, 3}, {0, 4}});
    ...
    SECTION("chain graph") {
        vod_sourced_void g({{0, 1}, {1, 2}, {2, 3}, {3, 4}});
    ...
    SECTION("cycle graph") {
        vod_sourced_void g({{0, 1}, {1, 2}, {2, 3}, {3, 0}});
    ...
    SECTION("with all value types") {
        vod_sourced_all g({{0, 1, 100}, {1, 2, 200}, {2, 0, 300}});
...
TEST_CASE("vod CPO source(g, uv)", "[dynamic_graph][vod][cpo][source]") {
    SECTION("basic usage") {
        vod_sourced_void g({{0, 1}, {1, 2}, {0, 2}});
    ...
    SECTION("returns valid descriptor") {
        vod_sourced_void g({{0, 1}, {1, 2}, {2, 0}});
    ...
    SECTION("with vertex values") {
        vod_sourced_all g({{0, 1, 100}, {1, 2, 200}});
    ...
    SECTION("self-loops") {
        vod_sourced_void g({{0, 0}, {1, 1}, {2, 2}});
    ...
    SECTION("accumulate values from edges") {
        vod_sourced_all g({{0, 1, 100}, {1, 2, 200}, {2, 3, 300}});
...
TEST_CASE("vod CPO integration: modify vertex and edge values", "[dynamic_graph][vod][cpo][integration]") {
    vod_all_int g({{0, 1, 1}, {1, 2, 2}});
