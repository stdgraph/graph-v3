 ...
 *   NOTE: num_edges(g, u) and num_edges(g, uid) NOT supported with vofl_graph_traits
 ...
 *   (provides equivalent functionality to num_edges(g, u) for vofl)
 ...
 * supported with vofl_graph_traits because:
...
using vofl_sourced_void = dynamic_graph<void, void, void, uint32_t, true, vofl_graph_traits<void, void, void, uint32_t, true>>;
using vofl_sourced_int  = dynamic_graph<int, void, void, uint32_t, true, vofl_graph_traits<int, void, void, uint32_t, true>>;
using vofl_sourced_all  = dynamic_graph<int, int, int, uint32_t, true, vofl_graph_traits<int, int, int, uint32_t, true>>;

//==================================================================================================
// 1. vertices(g) CPO Tests
//==================================================================================================

TEST_CASE("vofl CPO vertices(g)", "[dynamic_graph][vofl][cpo][vertices]") {
...
TEST_CASE("vofl CPO num_vertices(g)", "[dynamic_graph][vofl][cpo][num_vertices]") {
    SECTION("empty graph") {
        vofl_void g;
...
TEST_CASE("vofl CPO find_vertex(g, uid)", "[dynamic_graph][vofl][cpo][find_vertex]") {
...
TEST_CASE("vofl CPO vertex_id(g, u)", "[dynamic_graph][vofl][cpo][vertex_id]") {
...
TEST_CASE("vofl CPO num_edges(g)", "[dynamic_graph][vofl][cpo][num_edges]") {
    SECTION("empty graph") {
        vofl_void g;
...
TEST_CASE("vofl CPO edges(g, uid)", "[dynamic_graph][vofl][cpo][edges]") {
...
TEST_CASE("vofl CPO contains_edge(g, u, v)", "[dynamic_graph][vofl][cpo][contains_edge]") {
    SECTION("edge exists") {
        vofl_void g({{0, 1}, {0, 2}, {1, 2}});
...
TEST_CASE("vofl CPO contains_edge(g, uid, vid)", "[dynamic_graph][vofl][cpo][contains_edge][uid_vid]") {
    SECTION("basic usage") {
        vofl_void g({{0, 1}, {0, 2}, {1, 2}, {2, 3}});
...
TEST_CASE("vofl CPO integration", "[dynamic_graph][vofl][cpo][integration]") {
...
TEST_CASE("vofl CPO partition_id(g, u)", "[dynamic_graph][vofl][cpo][partition_id]") {
    ...
    SECTION("with different graph types") {
        vofl_int_ev g1({{0, 1, 10}, {1, 2, 20}});
        vofl_all_int g2({{0, 1, 1}, {1, 2, 2}});
        vofl_string g3({{0, 1, "edge"}});
...
TEST_CASE("vofl CPO source_id(g, uv)", "[dynamic_graph][vofl][cpo][source_id]") {
    SECTION("basic usage") {
        vofl_sourced_void g({{0, 1}, {1, 2}, {0, 2}});
    ...
    SECTION("multiple edges from same source") {
        vofl_sourced_void g({{0, 1}, {0, 2}, {0, 3}});
    ...
    SECTION("different sources") {
        vofl_sourced_void g({{0, 1}, {1, 2}, {2, 3}});
    ...
    SECTION("with edge values") {
        vofl_sourced_int g({{0, 1, 10}, {1, 2, 20}, {2, 0, 30}});
    ...
    SECTION("const correctness") {
        const vofl_sourced_void g({{0, 1}, {1, 2}});
    ...
    SECTION("parallel edges") {
        vofl_sourced_int g({{0, 1, 10}, {0, 1, 20}, {0, 1, 30}});
    ...
    SECTION("star graph") {
        vofl_sourced_void g({{0, 1}, {0, 2}, {0, 3}, {0, 4}});
    ...
    SECTION("chain graph") {
        vofl_sourced_void g({{0, 1}, {1, 2}, {2, 3}, {3, 4}});
    ...
    SECTION("cycle graph") {
        vofl_sourced_void g({{0, 1}, {1, 2}, {2, 3}, {3, 0}});
    ...
    SECTION("with all value types") {
        vofl_sourced_all g({{0, 1, 100}, {1, 2, 200}, {2, 0, 300}});
...
TEST_CASE("vofl CPO source(g, uv)", "[dynamic_graph][vofl][cpo][source]") {
    ...
    SECTION("returns valid descriptor") {
        vofl_sourced_void g({{0, 1}, {1, 2}, {2, 0}});
    ...
    SECTION("with vertex values") {
        vofl_sourced_all g({{0, 1, 100}, {1, 2, 200}});
    ...
    SECTION("self-loops") {
        vofl_sourced_void g({{0, 0}, {1, 1}, {2, 2}});
    ...
    SECTION("accumulate values from edges") {
        vofl_sourced_all g({{0, 1, 100}, {1, 2, 200}, {2, 3, 300}});
