...
using mol_void_void_void = dynamic_graph<void, void, void, uint32_t, false, mol_graph_traits<void, void, void, uint32_t, false>>;
using mol_int_void_void = dynamic_graph<int, void, void, uint32_t, false, mol_graph_traits<int, void, void, uint32_t, false>>;
using mol_void_int_void = dynamic_graph<void, int, void, uint32_t, false, mol_graph_traits<void, int, void, uint32_t, false>>;
using mol_int_int_void = dynamic_graph<int, int, void, uint32_t, false, mol_graph_traits<int, int, void, uint32_t, false>>;
using mol_void_void_int = dynamic_graph<void, void, int, uint32_t, false, mol_graph_traits<void, void, int, uint32_t, false>>;
...
using mol_str_void_void_void = dynamic_graph<void, void, void, std::string, false, mol_graph_traits<void, void, void, std::string, false>>;
using mol_str_int_void_void = dynamic_graph<int, void, void, std::string, false, mol_graph_traits<int, void, void, std::string, false>>;
using mol_str_void_int_void = dynamic_graph<void, int, void, std::string, false, mol_graph_traits<void, int, void, std::string, false>>;
using mol_str_int_int_int = dynamic_graph<int, int, int, std::string, false, mol_graph_traits<int, int, int, std::string, false>>;

using mol_sourced = dynamic_graph<void, void, void, uint32_t, true, mol_graph_traits<void, void, void, uint32_t, true>>;
using mol_int_sourced = dynamic_graph<int, void, void, uint32_t, true, mol_graph_traits<int, void, void, uint32_t, true>>;
...
TEST_CASE("mol traits verification", "[dynamic_graph][mol][traits]") {
    ...
    SECTION("edges_type is std::list") {
        using traits = mol_graph_traits<void, void, void, uint32_t, false>;
        using edges_t = typename traits::edges_type;
        // std::list has push_back and push_front (unlike forward_list which lacks push_back)
    ...
    SECTION("vertex_id_type can be string") {
        using traits = mol_graph_traits<void, void, void, std::string, false>;
    ...
    SECTION("custom vertex_id_type") {
        using traits = mol_graph_traits<void, void, void, int64_t, false>;
...
TEST_CASE("mol iterator categories", "[dynamic_graph][mol][iterators]") {
    ...
    SECTION("graph is a range") {
        static_assert(std::ranges::range<mol_void_void_void>);
        static_assert(std::ranges::range<mol_int_int_int>);
        static_assert(std::ranges::range<mol_str_void_void_void>);
        REQUIRE(true);
    }
...
TEST_CASE("mol construction", "[dynamic_graph][mol][construction]") {
    SECTION("default constructor creates empty graph") {
        mol_void_void_void g;
        REQUIRE(g.size() == 0);
    }
    ...
    SECTION("copy constructor") {
        mol_int_int_int g1;
        mol_int_int_int g2(g1);
        REQUIRE(g2.size() == g1.size());
    }

    SECTION("move constructor") {
        mol_int_int_int g1;
        mol_int_int_int g2(std::move(g1));
        REQUIRE(g2.size() == 0);
    }

    SECTION("copy assignment") {
        mol_int_int_int g1, g2;
        g2 = g1;
        REQUIRE(g2.size() == g1.size());
    }
...
TEST_CASE("mol construction with string vertex IDs", "[dynamic_graph][mol][construction][string]") {
    SECTION("default constructor creates empty graph") {
        mol_str_void_void_void g;
        REQUIRE(g.size() == 0);
    }

    SECTION("default constructor with int edge values") {
        mol_str_int_void_void g;
        REQUIRE(g.size() == 0);
    }

    SECTION("default constructor with int vertex values") {
        mol_str_void_int_void g;
        REQUIRE(g.size() == 0);
    }

    SECTION("default constructor with all int values") {
        mol_str_int_int_int g;
        REQUIRE(g.size() == 0);
    }
}
...
TEST_CASE("mol properties", "[dynamic_graph][mol][properties]") {
...
TEST_CASE("mol properties with string IDs", "[dynamic_graph][mol][properties][string]") {
    SECTION("size() on empty graph") {
        mol_str_void_void_void g;
        REQUIRE(g.size() == 0);
    }
...
TEST_CASE("mol type aliases", "[dynamic_graph][mol][types]") {
...
TEST_CASE("mol initializer_list construction", "[dynamic_graph][mol][initializer_list]") {
    ...
    SECTION("sparse vertex IDs - only referenced vertices created") {
        ...
        G g({{100, 200}});
        REQUIRE(g.size() == 2);
        // With mol, only vertices 100 and 200 are created
...
TEST_CASE("mol initializer_list construction string IDs", "[dynamic_graph][mol][initializer_list][string]") {
    SECTION("single edge with string IDs") {
        using G = mol_str_void_void_void;
        G g({{"alice", "bob"}});
        REQUIRE(g.size() == 2);
    }

    SECTION("string IDs with edge values") {
        using G = mol_str_int_void_void;
...
TEST_CASE("mol graph value access", "[dynamic_graph][mol][graph_value]") {
    SECTION("graph_value() returns reference") {
        mol_void_void_int g(100);
    ...
    SECTION("const graph_value()") {
        const mol_void_void_int g(42);
        REQUIRE(g.graph_value() == 42);
    }

    SECTION("graph value preserved through copy") {
        mol_void_void_int g1(100);
        mol_void_void_int g2 = g1;
    ...
    SECTION("graph value preserved through move") {
        mol_void_void_int g1(100);
        mol_void_void_int g2 = std::move(g1);
        REQUIRE(g2.graph_value() == 100);
    }
}
...
TEST_CASE("mol graph iteration", "[dynamic_graph][mol][iteration]") {
...
TEST_CASE("mol edge cases", "[dynamic_graph][mol][edge_cases]") {
...
TEST_CASE("mol const correctness", "[dynamic_graph][mol][const]") {
...
TEST_CASE("mol memory management", "[dynamic_graph][mol][memory]") {
...
TEST_CASE("mol template instantiation", "[dynamic_graph][mol][compilation]") {
    [[maybe_unused]] mol_void_void_void g1;
    [[maybe_unused]] mol_int_void_void g2;
    [[maybe_unused]] mol_void_int_void g3;
...
TEST_CASE("mol sparse vertex behavior", "[dynamic_graph][mol][sparse]") {
...
TEST_CASE("mol contains_vertex", "[dynamic_graph][mol][accessor][contains_vertex]") {
...
TEST_CASE("mol try_find_vertex", "[dynamic_graph][mol][accessor][try_find_vertex]") {
...
TEST_CASE("mol vertex_at", "[dynamic_graph][mol][accessor][vertex_at]") {
    SECTION("uint32_t vertex IDs - found") {
        using G = mol_void_void_void;
        G g({{0, 1}, {1, 2}});
    ...
    SECTION("modify vertex through vertex_at") {
        using G = mol_void_int_void;  // has vertex value
        G g({{0, 1}});
...
TEST_CASE("mol load_vertices", "[dynamic_graph][mol][load_vertices]") {
    SECTION("uint32_t IDs - basic load") {
        using G = mol_void_int_void;  // vertex value = int
    ...
    SECTION("string IDs - basic load") {
        using G = mol_str_void_int_void;  // string keys, vertex value = int
...
TEST_CASE("mol load_edges explicit", "[dynamic_graph][mol][load_edges]") {
    SECTION("uint32_t IDs - basic load") {
        using G = mol_int_void_void;  // edge value = int
    ...
    SECTION("string IDs - basic load") {
        using G = mol_str_int_void_void;  // string keys, edge value = int
...
TEST_CASE("mol load_vertices and load_edges combined", "[dynamic_graph][mol][load]") {
    SECTION("load edges then vertices") {
        using G = mol_int_int_void;  // edge value = int, vertex value = int
        using vertex_data = copyable_vertex_t<uint32_t, int>;
        using edge_data = copyable_edge_t<uint32_t, int>;
    ...
    SECTION("string IDs - combined loading") {
        using G = mol_str_int_int_int;  // all values are int
        using vertex_data = copyable_vertex_t<std::string, int>;
        using edge_data = copyable_edge_t<std::string, int>;
...
//==================================================================================================
// 19. Bidirectional Edge Iteration (std::list specific)
//==================================================================================================

TEST_CASE("mol bidirectional edge iteration", "[dynamic_graph][mol][edges][bidirectional]") {
    SECTION("edges preserve insertion order") {
        ...
        // Order is 10, 20, 30 (first added appears first with std::list)
        REQUIRE(values[0] == 10);
        REQUIRE(values[1] == 20);
        REQUIRE(values[2] == 30);
    }
    
    SECTION("bidirectional edge container allows reverse access") {
        using G = mol_int_void_void;
        G g({{0, 1, 10}, {0, 2, 20}, {0, 3, 30}});
...
TEST_CASE("mol std::ranges integration", "[dynamic_graph][mol][ranges]") {
...
TEST_CASE("mol algorithm compatibility", "[dynamic_graph][mol][algorithms]") {
...
TEST_CASE("mol complete workflow scenarios", "[dynamic_graph][mol][workflow]") {
...
//==================================================================================================
// Summary: Phase 3.1 mol_graph_traits Tests
//
// This file tests mol_graph_traits (map vertices + list edges).
// 
...
//
// Key differences from mofl (map + forward_list):
// - std::list allows bidirectional edge iteration
// - Edge order is preserved: first added appears first (unlike forward_list where last added appears first)
//
