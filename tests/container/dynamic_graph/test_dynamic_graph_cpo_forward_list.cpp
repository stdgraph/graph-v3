/**
 * @file test_dynamic_graph_cpo_forward_list.cpp
 * @brief Consolidated CPO tests for forward_list edge containers (vofl, dofl, mofl)
 * 
 * Uses template infrastructure from graph_test_types.hpp to test all 3 container
 * types with a single set of test cases.
 * 
 * IMPORTANT: forward_list uses push_front() for edge insertion, so edges appear
 * in REVERSE order of insertion. Tests are adapted to account for this behavior.
 */
...
TEMPLATE_TEST_CASE("forward_list CPO vertices(g)", "[dynamic_graph][cpo][vertices]", vofl_tag, dofl_tag) {
...
TEMPLATE_TEST_CASE("forward_list CPO num_vertices(g)", "[dynamic_graph][cpo][num_vertices]", vofl_tag, dofl_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_vv = typename Types::int_vv;
...
TEMPLATE_TEST_CASE("forward_list CPO find_vertex(g, uid)", "[dynamic_graph][cpo][find_vertex]", vofl_tag, dofl_tag) {
  using Types      = graph_test_types<TestType>;
  using Graph_void = typename Types::void_type;
...
TEMPLATE_TEST_CASE("forward_list CPO vertex_id(g, u)", "[dynamic_graph][cpo][vertex_id]", vofl_tag, dofl_tag) {
  using Types      = graph_test_types<TestType>;
  using Graph_void = typename Types::void_type;
...
TEMPLATE_TEST_CASE("forward_list CPO num_edges(g)", "[dynamic_graph][cpo][num_edges]", vofl_tag, dofl_tag) {
  using Types      = graph_test_types<TestType>;
  using Graph_void = typename Types::void_type;
...
TEMPLATE_TEST_CASE("forward_list CPO has_edges(g)", "[dynamic_graph][cpo][has_edges]", vofl_tag, dofl_tag) {
  using Types      = graph_test_types<TestType>;
  using Graph_void = typename Types::void_type;
...
//==================================================================================================
// 7. num_edges(g, u) and num_edges(g, uid) CPO Tests - NOT APPLICABLE
//
// IMPORTANT LIMITATION: num_edges(g, u) and num_edges(g, uid) CPO overloads are NOT
// supported with forward_list containers because:
...
//
// To test num_edges(g, u), see test_dynamic_graph_cpo_random_access.cpp which tests
// containers with sized edge ranges (vov, vod, dov, dod, vol, dol).
//==================================================================================================
...
TEMPLATE_TEST_CASE("forward_list CPO edges(g, u)", "[dynamic_graph][cpo][edges]", vofl_tag, dofl_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  ...
  SECTION("with edge values") {
    ...
    REQUIRE(values[0] == 200); // reverse insertion order
  ...
  SECTION("multiple edges") {
    ...
    REQUIRE(targets[0] == 3); // reverse insertion order
    REQUIRE(targets[1] == 2);
    REQUIRE(targets[2] == 1);
  }
  ...
  SECTION("with string edge values") {
    ...
    // forward_list: edges in reverse insertion order
...
TEMPLATE_TEST_CASE("forward_list CPO edges(g, uid)", "[dynamic_graph][cpo][edges]", vofl_tag, dofl_tag) {
  using Types      = graph_test_types<TestType>;
  using Graph_int_ev = typename Types::int_ev;
  using Graph_void = typename Types::void_type;
  ...
  SECTION("with parallel edges") {
    ...
    REQUIRE(values[0] == 30); // reverse insertion order
    REQUIRE(values[1] == 20);
    REQUIRE(values[2] == 10);
  }
...
TEMPLATE_TEST_CASE("forward_list CPO degree(g, u)", "[dynamic_graph][cpo][degree]", vofl_tag, dofl_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
...
TEMPLATE_TEST_CASE("forward_list CPO target_id(g, uv)", "[dynamic_graph][cpo][target_id]", vofl_tag, dofl_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
  ...
  SECTION("basic access") {
    ...
    // forward_list: edges in reverse order, so (0,2) first, then (0,1)
    REQUIRE(it != edge_view.end());
    auto uv0 = *it;
    REQUIRE(target_id(g, uv0) == 2);

    ++it;
    auto uv1 = *it;
    REQUIRE(target_id(g, uv1) == 1);
  }
  ...
  SECTION("self-loop") {
    ...
    // forward_list: edges in reverse order, so (0,1) first, then (0,0) self-loop
    REQUIRE(target_id(g, *it) == 1);
    ++it;
    // Second edge (0,0) is self-loop
  ...
  SECTION("iteration order") {
    ...
    // Edges appear in reverse insertion order (forward_list uses push_front)
...
TEMPLATE_TEST_CASE("forward_list CPO target(g, uv)", "[dynamic_graph][cpo][target]", vofl_tag, dofl_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
  using Graph_int_vv = typename Types::int_vv;
  ...
  SECTION("basic access") {
    ...
    // Verify it's the correct vertex (forward_list: edges in reverse order, so 2 is first)
  ...
  SECTION("self-loop") {
    ...
    // forward_list: edges in reverse order, so (0,1) first, then (0,0)
    // First edge (0,1)
    auto uv0     = *it;
    auto target0 = target(g, uv0);
    REQUIRE(vertex_id(g, target0) == 1);

    ++it;
    // Second edge (0,0) - self-loop
    auto uv1     = *it;
    auto target1 = target(g, uv1);
    REQUIRE(vertex_id(g, target1) == 0); // Target is same as source
  }
...
TEMPLATE_TEST_CASE("forward_list CPO find_vertex_edge(g, uid, vid)",
...
TEMPLATE_TEST_CASE("forward_list CPO find_vertex_edge(g, u, v)",
                   "[dynamic_graph][cpo][find_vertex_edge]",
                   vofl_tag,
                   dofl_tag) {
  using Types        = graph_test_types<TestType>;
...
TEMPLATE_TEST_CASE("forward_list CPO contains_edge(g, uid, vid)",
...
TEMPLATE_TEST_CASE("forward_list CPO contains_edge(g, u, v)",
                   "[dynamic_graph][cpo][contains_edge]",
                   vofl_tag,
                   dofl_tag) {
  using Types        = graph_test_types<TestType>;
...
TEMPLATE_TEST_CASE("forward_list CPO vertex_value(g, u)", "[dynamic_graph][cpo][vertex_value]", vofl_tag, dofl_tag) {
  using Types         = graph_test_types<TestType>;
  using Graph_int_vv  = typename Types::int_vv;
  using Graph_string  = typename Types::string_type;
...
TEMPLATE_TEST_CASE("forward_list CPO edge_value(g, uv)", "[dynamic_graph][cpo][edge_value]", vofl_tag, dofl_tag) {
  using Types         = graph_test_types<TestType>;
  using Graph_int_ev  = typename Types::int_ev;
  using Graph_all_int = typename Types::all_int;
  using Graph_string  = typename Types::string_type;
  ...
  SECTION("with self-loop") {
    ...
    // forward_list: edges in reverse order
    REQUIRE(values.size() == 2);
    REQUIRE(values[0] == 10);
    REQUIRE(values[1] == 99);
  }
  ...
  SECTION("large graph") {
    ...
    auto u0 = *find_vertex(g, 0);
    // forward_list: edges in reverse order, so first is 99*10=990
    int expected = 990;
    for (auto uv : edges(g, u0)) {
...
TEMPLATE_TEST_CASE("forward_list CPO graph_value(g)", "[dynamic_graph][cpo][graph_value]", vofl_tag, dofl_tag) {
  using Types         = graph_test_types<TestType>;
  using Graph_all_int = typename Types::all_int;
  using Graph_string  = typename Types::string_type;
...
TEMPLATE_TEST_CASE("forward_list CPO partition_id(g, u)", "[dynamic_graph][cpo][partition_id]", vofl_tag, dofl_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
...
TEMPLATE_TEST_CASE("forward_list CPO num_partitions(g)", "[dynamic_graph][cpo][num_partitions]", vofl_tag, dofl_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
...
TEMPLATE_TEST_CASE("forward_list CPO vertices(g, pid)",
...
TEMPLATE_TEST_CASE("forward_list CPO num_vertices(g, pid)",
...
TEMPLATE_TEST_CASE("forward_list CPO source_id(g, uv)", "[dynamic_graph][cpo][source_id]", vofl_tag, dofl_tag) {
  using Types              = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
...
TEMPLATE_TEST_CASE("forward_list CPO source(g, uv)", "[dynamic_graph][cpo][source]", vofl_tag, dofl_tag) {
  using Types              = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
...
TEMPLATE_TEST_CASE("forward_list CPO integration", "[dynamic_graph][cpo][integration]", vofl_tag, dofl_tag) {
  using Types      = graph_test_types<TestType>;
  using Graph_void = typename Types::void_type;
...
TEMPLATE_TEST_CASE("forward_list CPO integration: values", "[dynamic_graph][cpo][integration]", vofl_tag, dofl_tag) {
  using Types         = graph_test_types<TestType>;
  using Graph_all_int = typename Types::all_int;
...
TEMPLATE_TEST_CASE("forward_list CPO integration: modify vertex and edge values",
