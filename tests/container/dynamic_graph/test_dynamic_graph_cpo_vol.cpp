/**
 * @file test_dynamic_graph_cpo_vol.cpp
 * @brief Phase 2 CPO tests for dynamic_graph with vol_graph_traits
 * 
 ...
 * 
 * Container: vector<vertex> + list<edge>
 * 
 ...
 *   NOTE: num_edges(g, u) and num_edges(g, uid) NOT supported with vol_graph_traits
 ...
 *   (provides equivalent functionality to num_edges(g, u) for vol)
 ...
 * supported with vol_graph_traits because:
...
using vol_void     = dynamic_graph<void, void, void, uint32_t, false, vol_graph_traits<void, void, void, uint32_t, false>>;
using vol_int_ev   = dynamic_graph<int, void, void, uint32_t, false, vol_graph_traits<int, void, void, uint32_t, false>>;
using vol_int_vv   = dynamic_graph<void, int, void, uint32_t, false, vol_graph_traits<void, int, void, uint32_t, false>>;
using vol_all_int  = dynamic_graph<int, int, int, uint32_t, false, vol_graph_traits<int, int, int, uint32_t, false>>;
using vol_string   = dynamic_graph<std::string, std::string, std::string, uint32_t, false, 
...
using vol_sourced_void = dynamic_graph<void, void, void, uint32_t, true, vol_graph_traits<void, void, void, uint32_t, true>>;
using vol_sourced_int  = dynamic_graph<int, void, void, uint32_t, true, vol_graph_traits<int, void, void, uint32_t, true>>;
using vol_sourced_all  = dynamic_graph<int, int, int, uint32_t, true, vol_graph_traits<int, int, int, uint32_t, true>>;

//==================================================================================================
// 1. vertices(g) CPO Tests
//==================================================================================================

TEST_CASE("vol CPO vertices(g)", "[dynamic_graph][vol][cpo][vertices]") {
...
TEST_CASE("vol CPO num_vertices(g)", "[dynamic_graph][vol][cpo][num_vertices]") {
    SECTION("empty graph") {
        vol_void g;
...
TEST_CASE("vol CPO find_vertex(g, uid)", "[dynamic_graph][vol][cpo][find_vertex]") {
...
TEST_CASE("vol CPO vertex_id(g, u)", "[dynamic_graph][vol][cpo][vertex_id]") {
...
TEST_CASE("vol CPO num_edges(g)", "[dynamic_graph][vol][cpo][num_edges]") {
    SECTION("empty graph") {
        vol_void g;
...
TEST_CASE("vol CPO edges(g, u)", "[dynamic_graph][vol][cpo][edges]") {
...
TEST_CASE("vol CPO edges(g, uid)", "[dynamic_graph][vol][cpo][edges]") {
...
TEST_CASE("vol CPO degree(g, u)", "[dynamic_graph][vol][cpo][degree]") {
...
TEST_CASE("vol CPO target_id(g, uv)", "[dynamic_graph][vol][cpo][target_id]") {
    SECTION("basic access") {
        vol_void g({{0, 1}, {0, 2}, {1, 2}});
    ...
    SECTION("with edge values") {
        vol_int_ev g({{0, 1, 100}, {0, 2, 200}, {1, 2, 300}});
    ...
    SECTION("self-loop") {
        vol_void g({{0, 0}, {0, 1}});  // Self-loop and regular edge
    ...
    SECTION("with string edge values") {
        using vol_string_ev = dynamic_graph<std::string, void, void, uint32_t, false, 
...
TEST_CASE("vol CPO target(g, uv)", "[dynamic_graph][vol][cpo][target]") {
    SECTION("basic access") {
        vol_void g({{0, 1}, {0, 2}, {1, 2}});
...
TEST_CASE("vol CPO find_vertex_edge(g, u, v)", "[dynamic_graph][vol][cpo][find_vertex_edge]") {
    ...
    SECTION("with self-loop") {
        vol_void g({{0, 0}, {0, 1}});  // 0->0 (self-loop), 0->1
...
TEST_CASE("vol CPO find_vertex_edge(g, uid, vid)", "[dynamic_graph][vol][cpo][find_vertex_edge][uid_vid]") {
    SECTION("basic usage") {
        vol_void g({{0, 1}, {0, 2}, {1, 2}, {2, 3}});
...
TEST_CASE("vol CPO contains_edge(g, u, v)", "[dynamic_graph][vol][cpo][contains_edge]") {
    SECTION("edge exists") {
        vol_void g({{0, 1}, {0, 2}, {1, 2}});
...
TEST_CASE("vol CPO contains_edge(g, uid, vid)", "[dynamic_graph][vol][cpo][contains_edge][uid_vid]") {
    SECTION("basic usage") {
        vol_void g({{0, 1}, {0, 2}, {1, 2}, {2, 3}});
...
TEST_CASE("vol CPO integration", "[dynamic_graph][vol][cpo][integration]") {
...
TEST_CASE("vol CPO has_edge(g)", "[dynamic_graph][vol][cpo][has_edge]") {
    SECTION("empty graph") {
        vol_void g;
...
TEST_CASE("vol CPO vertex_value(g, u)", "[dynamic_graph][vol][cpo][vertex_value]") {
    SECTION("basic access") {
        vol_int_vv g;
        g.resize_vertices(3);
...
TEST_CASE("vol CPO edge_value(g, uv)", "[dynamic_graph][vol][cpo][edge_value]") {
    SECTION("basic access") {
        vol_int_ev g({{0, 1, 42}, {1, 2, 99}});
...
TEST_CASE("vol CPO integration: values", "[dynamic_graph][vol][cpo][integration]") {
...
TEST_CASE("vol CPO graph_value(g)", "[dynamic_graph][vol][cpo][graph_value]") {
    SECTION("basic access") {
        vol_all_int g({{0, 1, 1}});
...
TEST_CASE("vol CPO partition_id(g, u)", "[dynamic_graph][vol][cpo][partition_id]") {
    ...
    SECTION("with different graph types") {
        vol_int_ev g1({{0, 1, 10}, {1, 2, 20}});
        vol_all_int g2({{0, 1, 1}, {1, 2, 2}});
        vol_string g3({{0, 1, "edge"}});
...
TEST_CASE("vol CPO num_partitions(g)", "[dynamic_graph][vol][cpo][num_partitions]") {
    ...
    SECTION("const correctness") {
        const vol_void g({{0, 1}});
        
        REQUIRE(num_partitions(g) == 1);
    }

    SECTION("consistency with partition_id") {
        vol_all_int g({{0, 1, 1}, {1, 2, 2}, {2, 3, 3}});
...
TEST_CASE("vol CPO vertices(g, pid)", "[dynamic_graph][vol][cpo][vertices][partition]") {
...
TEST_CASE("vol CPO num_vertices(g, pid)", "[dynamic_graph][vol][cpo][num_vertices][partition]") {
...
TEST_CASE("vol CPO source_id(g, uv)", "[dynamic_graph][vol][cpo][source_id]") {
    SECTION("basic usage") {
        vol_sourced_void g({{0, 1}, {1, 2}, {0, 2}});
    ...
    SECTION("multiple edges from same source") {
        vol_sourced_void g({{0, 1}, {0, 2}, {0, 3}});
    ...
    SECTION("different sources") {
        vol_sourced_void g({{0, 1}, {1, 2}, {2, 3}});
    ...
    SECTION("with edge values") {
        vol_sourced_int g({{0, 1, 10}, {1, 2, 20}, {2, 0, 30}});
    ...
    SECTION("const correctness") {
        const vol_sourced_void g({{0, 1}, {1, 2}});
    ...
    SECTION("parallel edges") {
        vol_sourced_int g({{0, 1, 10}, {0, 1, 20}, {0, 1, 30}});
    ...
    SECTION("star graph") {
        vol_sourced_void g({{0, 1}, {0, 2}, {0, 3}, {0, 4}});
    ...
    SECTION("chain graph") {
        vol_sourced_void g({{0, 1}, {1, 2}, {2, 3}, {3, 4}});
    ...
    SECTION("cycle graph") {
        vol_sourced_void g({{0, 1}, {1, 2}, {2, 3}, {3, 0}});
    ...
    SECTION("with all value types") {
        vol_sourced_all g({{0, 1, 100}, {1, 2, 200}, {2, 0, 300}});
...
TEST_CASE("vol CPO source(g, uv)", "[dynamic_graph][vol][cpo][source]") {
    SECTION("basic usage") {
        vol_sourced_void g({{0, 1}, {1, 2}, {0, 2}});
    ...
    SECTION("returns valid descriptor") {
        vol_sourced_void g({{0, 1}, {1, 2}, {2, 0}});
    ...
    SECTION("with vertex values") {
        vol_sourced_all g({{0, 1, 100}, {1, 2, 200}});
    ...
    SECTION("self-loops") {
        vol_sourced_void g({{0, 0}, {1, 1}, {2, 2}});
    ...
    SECTION("accumulate values from edges") {
        vol_sourced_all g({{0, 1, 100}, {1, 2, 200}, {2, 3, 300}});
...
TEST_CASE("vol CPO integration: modify vertex and edge values", "[dynamic_graph][vol][cpo][integration]") {
    vol_all_int g({{0, 1, 1}, {1, 2, 2}});
