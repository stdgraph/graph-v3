/**
 * @file test_dynamic_graph_uol.cpp
 * @brief Tests for dynamic_graph with unordered_map vertices + list edges
 * 
 * Phase 3.1f: Unordered Map Vertex Containers
 * Tests uol_graph_traits (unordered_map vertices + list edges)
 ...
 * This file focuses on uol_graph_traits specific functionality.
 */
...
using uol_void_void_void = dynamic_graph<void, void, void, uint32_t, false, uol_graph_traits<void, void, void, uint32_t, false>>;
using uol_int_void_void = dynamic_graph<int, void, void, uint32_t, false, uol_graph_traits<int, void, void, uint32_t, false>>;
using uol_void_int_void = dynamic_graph<void, int, void, uint32_t, false, uol_graph_traits<void, int, void, uint32_t, false>>;
using uol_int_int_void = dynamic_graph<int, int, void, uint32_t, false, uol_graph_traits<int, int, void, uint32_t, false>>;
using uol_void_void_int = dynamic_graph<void, void, int, uint32_t, false, uol_graph_traits<void, void, int, uint32_t, false>>;
using uol_int_int_int = dynamic_graph<int, int, int, uint32_t, false, uol_graph_traits<int, int, int, uint32_t, false>>;

// Type aliases with string vertex IDs (a common use case for unordered_map containers)
using uol_str_void_void_void = dynamic_graph<void, void, void, std::string, false, uol_graph_traits<void, void, void, std::string, false>>;
using uol_str_int_void_void = dynamic_graph<int, void, void, std::string, false, uol_graph_traits<int, void, void, std::string, false>>;
using uol_str_void_int_void = dynamic_graph<void, int, void, std::string, false, uol_graph_traits<void, int, void, std::string, false>>;
using uol_str_int_int_int = dynamic_graph<int, int, int, std::string, false, uol_graph_traits<int, int, int, std::string, false>>;

using uol_sourced = dynamic_graph<void, void, void, uint32_t, true, uol_graph_traits<void, void, void, uint32_t, true>>;
using uol_int_sourced = dynamic_graph<int, void, void, uint32_t, true, uol_graph_traits<int, void, void, uint32_t, true>>;
...
TEST_CASE("uol traits verification", "[dynamic_graph][uol][traits]") {
    ...
    SECTION("edges_type is std::list") {
        ...
        // list has bidirectional iterators (rbegin/rend)
        static_assert(requires(edges_t e) { e.rbegin(); e.rend(); });
        // list has both push_front and push_back (unlike forward_list)
        static_assert(requires(edges_t e, typename edges_t::value_type v) { e.push_front(v); e.push_back(v); });
        REQUIRE(true);
    }

    SECTION("vertex_id_type can be string") {
        using traits = uol_graph_traits<void, void, void, std::string, false>;
    ...
    SECTION("custom vertex_id_type") {
        using traits = uol_graph_traits<void, void, void, int64_t, false>;
...
TEST_CASE("uol iterator categories", "[dynamic_graph][uol][iterators]") {
    ...
    SECTION("edge descriptor iterators are forward only") {
        using traits = uol_graph_traits<void, void, void, uint32_t, false>;
        using edges_t = typename traits::edges_type;
        // Underlying list has bidirectional iterators
    ...
    SECTION("graph is a range") {
        static_assert(std::ranges::range<uol_void_void_void>);
        static_assert(std::ranges::range<uol_int_int_int>);
        static_assert(std::ranges::range<uol_str_void_void_void>);
        REQUIRE(true);
    }
}
...
TEST_CASE("uol construction", "[dynamic_graph][uol][construction]") {
    SECTION("default constructor creates empty graph") {
        uol_void_void_void g;
        REQUIRE(g.size() == 0);
    }
    ...
    SECTION("copy constructor") {
        uol_int_int_int g1;
        uol_int_int_int g2(g1);
        REQUIRE(g2.size() == g1.size());
    }

    SECTION("move constructor") {
        uol_int_int_int g1;
        uol_int_int_int g2(std::move(g1));
        REQUIRE(g2.size() == 0);
    }

    SECTION("copy assignment") {
        uol_int_int_int g1, g2;
        g2 = g1;
        REQUIRE(g2.size() == g1.size());
    }
...
TEST_CASE("uol construction with string vertex IDs", "[dynamic_graph][uol][construction][string]") {
    SECTION("default constructor creates empty graph") {
        uol_str_void_void_void g;
        REQUIRE(g.size() == 0);
    }

    SECTION("default constructor with int edge values") {
        uol_str_int_void_void g;
        REQUIRE(g.size() == 0);
    }

    SECTION("default constructor with int vertex values") {
        uol_str_void_int_void g;
        REQUIRE(g.size() == 0);
    }

    SECTION("default constructor with all int values") {
        uol_str_int_int_int g;
        REQUIRE(g.size() == 0);
    }
}
...
TEST_CASE("uol properties", "[dynamic_graph][uol][properties]") {
...
TEST_CASE("uol properties with string IDs", "[dynamic_graph][uol][properties][string]") {
    SECTION("size() on empty graph") {
        uol_str_void_void_void g;
        REQUIRE(g.size() == 0);
    }
...
TEST_CASE("uol type aliases", "[dynamic_graph][uol][types]") {
...
TEST_CASE("uol initializer_list construction", "[dynamic_graph][uol][initializer_list]") {
    ...
    SECTION("sparse vertex IDs - only referenced vertices created") {
        ...
        G g({{100, 200}});
        REQUIRE(g.size() == 2);
        // With uol, only vertices 100 and 200 are created
...
TEST_CASE("uol initializer_list construction string IDs", "[dynamic_graph][uol][initializer_list][string]") {
    SECTION("single edge with string IDs") {
        using G = uol_str_void_void_void;
        G g({{"alice", "bob"}});
        REQUIRE(g.size() == 2);
    }

    SECTION("string IDs with edge values") {
        using G = uol_str_int_void_void;
...
TEST_CASE("uol graph value access", "[dynamic_graph][uol][graph_value]") {
    SECTION("graph_value() returns reference") {
        uol_void_void_int g(100);
    ...
    SECTION("const graph_value()") {
        const uol_void_void_int g(42);
        REQUIRE(g.graph_value() == 42);
    }

    SECTION("graph value preserved through copy") {
        uol_void_void_int g1(100);
        uol_void_void_int g2 = g1;
    ...
    SECTION("graph value preserved through move") {
        uol_void_void_int g1(100);
        uol_void_void_int g2 = std::move(g1);
        REQUIRE(g2.graph_value() == 100);
    }
}
...
TEST_CASE("uol graph iteration", "[dynamic_graph][uol][iteration]") {
    ...
    SECTION("iteration order is unspecified (hash-based)") {
        // Unlike mofl (map), uol does NOT iterate in key order
        using G = uol_void_void_void;
        G g({{5, 10}, {1, 2}, {3, 4}});
...
TEST_CASE("uol edge cases", "[dynamic_graph][uol][edge_cases]") {
...
TEST_CASE("uol const correctness", "[dynamic_graph][uol][const]") {
...
TEST_CASE("uol memory management", "[dynamic_graph][uol][memory]") {
...
TEST_CASE("uol template instantiation", "[dynamic_graph][uol][compilation]") {
    [[maybe_unused]] uol_void_void_void g1;
    [[maybe_unused]] uol_int_void_void g2;
    [[maybe_unused]] uol_void_int_void g3;
...
TEST_CASE("uol sparse vertex behavior", "[dynamic_graph][uol][sparse]") {
...
TEST_CASE("uol contains_vertex", "[dynamic_graph][uol][accessor][contains_vertex]") {
...
TEST_CASE("uol try_find_vertex", "[dynamic_graph][uol][accessor][try_find_vertex]") {
...
TEST_CASE("uol vertex_at", "[dynamic_graph][uol][accessor][vertex_at]") {
    SECTION("uint32_t vertex IDs - found") {
        using G = uol_void_void_void;
        G g({{0, 1}, {1, 2}});
    ...
    SECTION("modify vertex through vertex_at") {
        using G = uol_void_int_void;  // has vertex value
        G g({{0, 1}});
...
TEST_CASE("uol load_vertices", "[dynamic_graph][uol][load_vertices]") {
    SECTION("uint32_t IDs - basic load") {
        using G = uol_void_int_void;  // vertex value = int
    ...
    SECTION("string IDs - basic load") {
        using G = uol_str_void_int_void;  // string keys, vertex value = int
...
TEST_CASE("uol load_edges explicit", "[dynamic_graph][uol][load_edges]") {
    SECTION("uint32_t IDs - basic load") {
        using G = uol_int_void_void;  // edge value = int
    ...
    SECTION("string IDs - basic load") {
        using G = uol_str_int_void_void;  // string keys, edge value = int
...
TEST_CASE("uol load_vertices and load_edges combined", "[dynamic_graph][uol][load]") {
    SECTION("load edges then vertices") {
        using G = uol_int_int_void;  // edge value = int, vertex value = int
        using vertex_data = copyable_vertex_t<uint32_t, int>;
        using edge_data = copyable_edge_t<uint32_t, int>;
    ...
    SECTION("string IDs - combined loading") {
        using G = uol_str_int_int_int;  // all values are int
        using vertex_data = copyable_vertex_t<std::string, int>;
        using edge_data = copyable_edge_t<std::string, int>;
...
TEST_CASE("uol hash-specific behavior", "[dynamic_graph][uol][hash]") {
...
//==================================================================================================
// Summary: Phase 3.1f uol_graph_traits Tests
//
// This file tests uol_graph_traits (unordered_map vertices + list edges).
// 
