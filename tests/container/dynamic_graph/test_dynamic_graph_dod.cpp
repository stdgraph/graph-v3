...
using dod_void_void_void = dod_graph<>;
using dod_int_void_void = dod_graph<int>;
using dod_void_int_void = dod_graph<void, int>;
using dod_int_int_void = dod_graph<int, int>;
using dod_void_void_int = dod_graph<void, void, int>;
using dod_int_int_int = dod_graph<int, int, int>;

using dod_string_string_string = dod_graph<std::string, std::string, std::string>;
...
TEST_CASE("dod value types", "[dod][value_types]") {
  SECTION("with void edge value") {
    using graph_t = dod_graph<void, int, int>;
    graph_t g(100);
    REQUIRE(g.graph_value() == 100);
  }

  SECTION("with void vertex value") {
    using graph_t = dod_graph<int, void, int>;
    graph_t g(100);
    REQUIRE(g.graph_value() == 100);
  }

  SECTION("with void graph value") {
    using graph_t = dod_graph<int, int>;
    graph_t g;
    REQUIRE(g.size() == 0);
  }
  ...
  SECTION("with string edge value type") {
    using graph_t = dod_graph<std::string>;
    graph_t g;
    REQUIRE(g.size() == 0);
  }

  SECTION("with string vertex value type") {
    using graph_t = dod_graph<void, std::string>;
    graph_t g;
    REQUIRE(g.size() == 0);
  }

  SECTION("with string graph value type") {
    using graph_t = dod_graph<void, void, std::string>;
...
TEST_CASE("dod vertex ID types", "[dod][vertex_id]") {
  ...
  SECTION("with uint64_t vertex id") {
    using graph_t =
          dod_graph<void, void, void, uint64_t>;
    graph_t g;
    REQUIRE(g.size() == 0);
  }

  SECTION("with int32_t vertex id") {
    using graph_t = dod_graph<void, void, void, int32_t>;
    graph_t g;
    REQUIRE(g.size() == 0);
  }

  SECTION("with int8_t vertex id") {
    using graph_t = dod_graph<void, void, void, int8_t>;
    graph_t g;
    REQUIRE(g.size() == 0);
  }

  SECTION("with int vertex id") {
    using graph_t = dod_graph<void, void, void, int>;
    graph_t g;
    REQUIRE(g.size() == 0);
  }
}
...
TEST_CASE("dod load_vertices", "[dynamic_graph][dod][load_vertices]") {
  ...
  SECTION("with custom projection from struct") {
    using G2           = dod_graph<int, std::string>;
...
TEST_CASE("dod load_edges", "[dynamic_graph][dod][load_edges]") {
  ...
  SECTION("with custom projection") {
    using G2           = dod_graph<std::string, int>;
    using vertex_data2 = copyable_vertex_t<uint32_t, int>;
