/**
 * @file test_dynamic_graph_cpo_sorted.cpp
 * @brief Consolidated CPO tests for sorted edge containers (vos, dos)
 * 
 * Uses template infrastructure from graph_test_types.hpp to test container
 * types with a single set of test cases.
 * 
 * NOTE: mos and uos (map-based vertex containers) are NOT included here
 * because they use different vertex creation semantics (on-demand vertex
 * creation from edges rather than resize_vertices).
 * 
 * IMPORTANT: Set containers maintain edges in sorted order by target_id. Tests
 * are adapted to account for this behavior - edges will be encountered in
 * target_id ascending order regardless of insertion order.
 */
...
TEMPLATE_TEST_CASE("sorted CPO vertices(g)", "[dynamic_graph][cpo][vertices]", vos_tag, dos_tag) {
...
TEMPLATE_TEST_CASE("sorted CPO num_vertices(g)", "[dynamic_graph][cpo][num_vertices]", vos_tag, dos_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_vv = typename Types::int_vv;
...
TEMPLATE_TEST_CASE("sorted CPO find_vertex(g, uid)", "[dynamic_graph][cpo][find_vertex]", vos_tag, dos_tag) {
  using Types      = graph_test_types<TestType>;
  using Graph_void = typename Types::void_type;
...
TEMPLATE_TEST_CASE("sorted CPO vertex_id(g, u)", "[dynamic_graph][cpo][vertex_id]", vos_tag, dos_tag) {
  using Types      = graph_test_types<TestType>;
  using Graph_void = typename Types::void_type;
...
TEMPLATE_TEST_CASE("sorted CPO num_edges(g)", "[dynamic_graph][cpo][num_edges]", vos_tag, dos_tag) {
  using Types      = graph_test_types<TestType>;
  using Graph_void = typename Types::void_type;
...
TEMPLATE_TEST_CASE("sorted CPO num_edges(g, u)", "[dynamic_graph][cpo][num_edges]", vos_tag, dos_tag) {
  using Types      = graph_test_types<TestType>;
  using Graph_void = typename Types::void_type;
...
TEMPLATE_TEST_CASE("sorted CPO num_edges(g, uid)", "[dynamic_graph][cpo][num_edges]", vos_tag, dos_tag) {
  using Types      = graph_test_types<TestType>;
  using Graph_void = typename Types::void_type;
...
TEMPLATE_TEST_CASE("sorted CPO edges(g, u)", "[dynamic_graph][cpo][edges]", vos_tag, dos_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  ...
  SECTION("with parallel edges") {
    ...
    // Set containers deduplicate, so only 1 edge remains
    REQUIRE(count == 1); // Set deduplicates
  }
...
TEMPLATE_TEST_CASE("sorted CPO edges(g, uid)", "[dynamic_graph][cpo][edges]", vos_tag, dos_tag) {
  using Types      = graph_test_types<TestType>;
  using Graph_void = typename Types::void_type;
  ...
  SECTION("with parallel edges") {
    ...
    std::vector<copyable_edge_t<uint32_t, int>> edge_data = {
          {0, 1, 10}, {0, 1, 20}, {0, 1, 30} // 3 parallel edges, set deduplicates
    };
    g.load_edges(edge_data);
    ...
    // Set containers deduplicate by target_id, so only 1 edge remains
...
TEMPLATE_TEST_CASE("sorted CPO degree(g, u)", "[dynamic_graph][cpo][degree]", vos_tag, dos_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
...
TEMPLATE_TEST_CASE("sorted CPO target_id(g, uv)", "[dynamic_graph][cpo][target_id]", vos_tag, dos_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
  ...
  SECTION("iteration order") {
    ...
    // Edges appear in sorted order by target_id (ascending)
...
TEMPLATE_TEST_CASE("sorted CPO target(g, uv)", "[dynamic_graph][cpo][target]", vos_tag, dos_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
  using Graph_int_vv = typename Types::int_vv;
...
TEMPLATE_TEST_CASE("sorted CPO find_vertex_edge(g, uid, vid)",
...
TEMPLATE_TEST_CASE("sorted CPO find_vertex_edge(g, u, v)", "[dynamic_graph][cpo][find_vertex_edge]", vos_tag, dos_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
  using Graph_string = typename Types::string_type;
...
TEMPLATE_TEST_CASE("sorted CPO contains_edge(g, uid, vid)", "[dynamic_graph][cpo][contains_edge]", vos_tag, dos_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
...
TEMPLATE_TEST_CASE("sorted CPO contains_edge(g, u, v)", "[dynamic_graph][cpo][contains_edge]", vos_tag, dos_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
...
TEMPLATE_TEST_CASE("sorted CPO vertex_value(g, u)", "[dynamic_graph][cpo][vertex_value]", vos_tag, dos_tag) {
  using Types         = graph_test_types<TestType>;
  using Graph_int_vv  = typename Types::int_vv;
  using Graph_string  = typename Types::string_type;
...
TEMPLATE_TEST_CASE("sorted CPO edge_value(g, uv)", "[dynamic_graph][cpo][edge_value]", vos_tag, dos_tag) {
  using Types         = graph_test_types<TestType>;
  using Graph_int_ev  = typename Types::int_ev;
  using Graph_all_int = typename Types::all_int;
  using Graph_string  = typename Types::string_type;
  ...
  SECTION("with parallel edges") {
    ...
    std::vector<copyable_edge_t<uint32_t, int>> edge_data = {
          {0, 1, 10}, {0, 1, 20}, {0, 1, 30} // Set deduplicates
    };
    g.load_edges(edge_data);
    ...
    // Set containers deduplicate by target_id
    REQUIRE(values.size() == 1);
  }
...
TEMPLATE_TEST_CASE("sorted CPO graph_value(g)", "[dynamic_graph][cpo][graph_value]", vos_tag, dos_tag) {
  using Types         = graph_test_types<TestType>;
  using Graph_all_int = typename Types::all_int;
  using Graph_string  = typename Types::string_type;
...
TEMPLATE_TEST_CASE("sorted CPO partition_id(g, u)", "[dynamic_graph][cpo][partition_id]", vos_tag, dos_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
...
TEMPLATE_TEST_CASE("sorted CPO num_partitions(g)", "[dynamic_graph][cpo][num_partitions]", vos_tag, dos_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
...
TEMPLATE_TEST_CASE("sorted CPO vertices(g, pid)", "[dynamic_graph][cpo][vertices][partition]", vos_tag, dos_tag) {
  using Types        = graph_test_types<TestType>;
  using Graph_void   = typename Types::void_type;
  using Graph_int_ev = typename Types::int_ev;
...
TEMPLATE_TEST_CASE("sorted CPO num_vertices(g, pid)",
...
TEMPLATE_TEST_CASE("sorted CPO source_id(g, uv)", "[dynamic_graph][cpo][source_id]", vos_tag, dos_tag) {
  using Types              = graph_test_types<TestType>;
...
TEMPLATE_TEST_CASE("sorted CPO source(g, uv)", "[dynamic_graph][cpo][source]", vos_tag, dos_tag) {
  using Types              = graph_test_types<TestType>;
  using Graph_sourced_void = typename Types::sourced_void;
  using Graph_sourced_int  = typename Types::sourced_int;
...
TEMPLATE_TEST_CASE("sorted CPO integration", "[dynamic_graph][cpo][integration]", vos_tag, dos_tag) {
  using Types      = graph_test_types<TestType>;
  using Graph_void = typename Types::void_type;
...
TEMPLATE_TEST_CASE("sorted CPO integration: values", "[dynamic_graph][cpo][integration]", vos_tag, dos_tag) {
  using Types         = graph_test_types<TestType>;
  using Graph_all_int = typename Types::all_int;
...
TEMPLATE_TEST_CASE("sorted CPO integration: modify vertex and edge values",
