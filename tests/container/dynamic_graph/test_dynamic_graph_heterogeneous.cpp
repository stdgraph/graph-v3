...
TEST_CASE("Visit variant graphs with std::visit", "[6.4.3]") {
  ...
  auto vertex_count = std::visit([](auto&& g) { return count_vertices(g); }, var);

  auto edge_count = std::visit([](auto&& g) { return count_edges(g); }, var);
...
TEST_CASE("Generic operations on variant collection", "[6.4.3]") {
  ...
  for (auto&& var : graphs) {
    total_edges += std::visit([](auto&& g) { return count_edges(g); }, var);
  }
...
TEST_CASE("Visit with overloaded lambdas", "[6.4.3]") {
  ...
  auto process = overloaded{[](vov_uint& g) { return count_vertices(g) * 10; },
...
TEST_CASE("Exception safety with variant graphs", "[6.4.3]") {
  ...
  REQUIRE_NOTHROW(std::visit([](auto&& graph_arg) { return count_vertices(graph_arg); }, var));
...
TEST_CASE("Vector of heterogeneous graphs with complex operations", "[6.4.3]") {
  ...
  std::vector<copyable_edge_string>   edges2    = {{"center", "leaf1"}, {"center", "leaf2"}, {"center", "leaf3"}};
  std::vector<copyable_vertex_string> vertices2 = {{"center"}, {"leaf1"}, {"leaf2"}, {"leaf3"}};
  std::vector<std::string>            partitions2;
  graphs.emplace_back(mos_string(edges2, vertices2, identity{}, identity{}, partitions2));
  ...
  for (size_t i = 0; i < graphs.size(); ++i) {
    auto edge_count = std::visit([](auto&& g) { return count_edges(g); }, graphs[i]);
