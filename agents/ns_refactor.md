...
in the `graph` namespace and there is no separate location for an edge list. 
Abstract edge lists are a peer of adjacency lists, not a subset or refinement to them.

The GCI (Graph Container Interface) related to concepts, types, CPOs, and Descriptors
that are used to define a general interace that can adapt to any adjacency list.

The abstract edge list, views and algorithms have not been defined yet, so any reference
to them can be ignored for now.

This is a new library so backward compatibility is not an issue.

Tests for the existing code should be refactored with the new namespace and pass without error.
...
│   └── graph::edge_list::views          # View factories for edge lists (TBD)
