...
template <std::ranges::random_access_range V,
          std::ranges::random_access_range E,
          class Graph = std::vector<std::vector<std::tuple<size_t, size_t>>>>
...
// Join two plain adjacency-list graphs (each stored as vector<vector<size_t>>).
//
// Given:
//   G[movie] -> list of actor IDs  (movie-to-actor graph)
//   H[actor] -> list of movie IDs  (actor-to-movie graph)
//
// Produces an actor-actor graph where edge {i, j, movie_k} means
// actor i and actor j co-starred in movie k.
//
// For plain integer-edge graphs the edge value in H[i] and G[k] IS the
// target ID, so we read it directly without going through graph CPOs.
...
auto join(const Graph1& G, const Graph2& H) {
  ...
  for (size_t i = 0; i < H.size(); ++i) {
    for (auto&& k : H[i]) {
      // k is the target vertex ID in H (a plain integer edge value)
      for (auto&& j : G[k]) {
        // j is the target vertex ID in G (a plain integer edge value)
        if (j != i) {
